    ...
    for (const [label, viewport] of [['desktop', { width: 1440, height: 960 }], ['mobile', { width: 390, height: 844 }], ['small-mobile', { width: 320, height: 740 }]]) {
      ...
      await context.route('**/*', async (route) => {
        ...
        if (url.pathname === '/api/xpace/finance') {
          ...
          return send({ success: true, canManage: true, metrics: receive ? { total: 22000, paid: 22000, open: 0, inProgress: 0 } : { total: 180000, paid: 30000, open: 150000, inProgress: 0 }, items, totalRows: items.length, page: 0, pageSize: 20, accounts: [{ id: 'account-test', description: 'Banco da escola' }] });
        }
