...
document.addEventListener('DOMContentLoaded', () => {
    ...
    updatePricingFormation();
        // ==========================================
    // XPACEBOX SAAS MULTI-MODULE ENGINE (v142)
    // ==========================================
    function initSaaSModules() {
        ...
        function switchModule(moduleName) {
            ...
            // Ocultar todas as secoes primeiro
        ...
        // --- GESTAO DE CLIENTES (CRUD & VIACEP) ---
        let clientsList = getStoredData('dawos_clients') || [
            {
                id: 'cli-1',
                codigo_unico: 'CLI-001',
                nome_real: 'Embalagens Brasil Industria e Comercio Ltda',
        ...
        if (btnOpenClientModal) {
            btnOpenClientModal.addEventListener('click', () => {
                ...
                if (modalClient) modalClient.classList.remove('hidden');
            });
        }
        ...
        function editClient(id) {
            ...
            document.getElementById('client-origem').value = client.origem || 'Indicacao';
            document.getElementById('client-ramo').value = client.ramo || 'Alimenticio';
            document.getElementById('client-representante').value = client.representante || 'Venda Direta / Fabrica';
        ...
        // --- GESTAO DE PRODUTOS / FICHAS TECNICAS ---
        ...
        function renderProductsTable(filterText = '') {
            ...
            if (filtered.length === 0) {
                tbody.innerHTML = '<tr><td colspan="9" style="text-align: center; padding: 24px; color: var(--color-text-muted);">Nenhuma Ficha Tecnica cadastrada.</td></tr>';
                return;
            }
        ...
        if (btnOpenProductModal) {
            btnOpenProductModal.addEventListener('click', () => {
                ...
                if (modalProduct) modalProduct.classList.remove('hidden');
            });
        }
        ...
        function deleteProduct(id) {
            if (confirm("Tem certeza que deseja excluir esta Ficha Tecnica?")) {
        ...
        if (btnTransferProductModal) {
            btnTransferProductModal.addEventListener('click', () => {
                if (productsList.length === 0) {
                    alert("Cadastre pelo menos uma Ficha Tecnica primeiro para realizar a transferencia.");
                    return;
                }
                ...
                document.getElementById('product-descricao').value = firstProd.descricao + ' (Copia)';
        ...
        // FUNCAO DE INTEGRACAO COM A FORMADORA DE PRECO
        function loadProductIntoPricing(id) {
            ...
            alert("âœ… Ficha TÃ©cnica " + prod.numero_ficha + " (" + prod.descricao + ") carregada com sucesso na Calculadora de PreÃ§os!");
        }
