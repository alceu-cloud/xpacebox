  ...
  return (
    <main className="xb-central">
      ...
      <section className="xb-company-grid" aria-label="Painéis disponíveis">
        ...
        {companies.map((company, index) => <button key={company.id} type="button" className="xb-company-card" onClick={() => openCompany(company)}><small>Empresa</small><strong>{company.name}</strong><span>Abrir painel</span><i aria-hidden="true">{String(index + (role === "platform_owner" ? 2 : 1)).padStart(2, "0")}</i></button>)}
