...
type Body = {
  ...
  group?: { id?: string; name?: string; modalityId?: string; level?: string; ageGroup?: string; roomId?: string; instructorId?: string; color?: string; capacity?: number; sourceType?: string; settings?: unknown; weekdays?: number[]; startsAt?: string; endsAt?: string; schedules?: Array<{ id?: string; weekday?: number; startsAt?: string; endsAt?: string; roomId?: string; instructorId?: string; ageGroup?: string; capacity?: number | string; settings?: unknown }> };
...
export async function GET(request: Request) {
  try {
    ...
    const [groupsResult, schedulesResult, enrollmentsResult, studentsResult, rentalsResult, modalitiesResult, instructorsResult, roomsResult] = await Promise.all([
      ...
      admin.from("xpace_class_schedules").select("id,class_group_id,weekday,starts_at,ends_at,room_name,room_id,instructor_id,age_group,capacity,settings,active").eq("tenant_company_id", company.id).eq("active", true).order("weekday").order("starts_at"),
    ...
    return NextResponse.json({ success: true, groups: (groupsResult.data ?? []).map((group) => ({ id: group.id, name: group.name, modality: group.modality ?? "", modalityId: group.modality_id ?? "", level: normalizeClassLevel(group.class_level), instructorName: group.instructor_id ? instructorNames.get(group.instructor_id) ?? "PROFESSOR ARQUIVADO" : "", color: group.color, capacity: group.capacity, sourceType: group.source_type, settings: group.settings ?? {}, active: group.active, schedules: (schedulesResult.data ?? []).filter((schedule) => schedule.class_group_id === group.id).map((schedule) => ({ id: schedule.id, weekday: schedule.weekday, startsAt: schedule.starts_at.slice(0, 5), endsAt: schedule.ends_at.slice(0, 5), roomName: schedule.room_name ?? "", roomId: schedule.room_id ?? "", instructorId: schedule.instructor_id ?? "", instructorName: schedule.instructor_id ? instructorNames.get(schedule.instructor_id) ?? "PROFESSOR ARQUIVADO" : "", ageGroup: normalizeAgeGroup(schedule.age_group) || "ADULTO", capacity: schedule.capacity ?? group.capacity, settings: schedule.settings ?? group.settings ?? {} })), students: enrollmentsByGroup.get(group.id) ?? [] })), modalities: (modalitiesResult.data ?? []).map((modality) => ({ id: modality.id, name: modality.name, instructorId: modality.instructor_id ?? "", instructorName: modality.instructor_id ? instructorNames.get(modality.instructor_id) ?? "PROFESSOR ARQUIVADO" : "", requiresInstructor: modality.requires_instructor })), instructors: (instructorsResult.data ?? []).map((instructor) => ({ id: instructor.id, fullName: instructor.full_name })), rooms: (roomsResult.data ?? []).map((room) => ({ id: room.id, name: room.name, capacity: room.capacity })), students: (studentsResult.data ?? []).map((student) => ({ id: student.id, name: student.full_name, mobile: student.mobile ?? "" })), rentals: (rentalsResult.data ?? []).map((rental) => ({ id: rental.id, roomName: rental.room_name, renterName: rental.renter_name, startsAt: rental.starts_at, endsAt: rental.ends_at, amountCents: rental.amount_cents, status: rental.status, note: rental.note ?? "" })) });
...
function normalizeGroup(input?: Body["group"]) { return { name: clean(input?.name), modalityId: clean(input?.modalityId), level: normalizeClassLevel(input?.level), ageGroup: normalizeAgeGroup(input?.ageGroup), color: clean(input?.color) || "#7435d9", sourceType: input?.sourceType === "SERVICO" ? "SERVICO" : "CONTRATO" }; }
...
async function prepareScheduleRows(access: Awaited<ReturnType<typeof requireCompanyAccess>>, input: Body["group"] | undefined, modalityId: string, excludedGroupId = "") {
  ...
  const rows = raw.map((slot) => {
    ...
    return { weekday: slot.weekday, starts_at: slot.startsAt, ends_at: slot.endsAt, room_name: room.name, room_id: room.id, instructor_id: slot.instructorId, age_group: slot.ageGroup, capacity: requestedCapacity, settings: slot.settings, active: true };
  });
