...
type Body = {
  ...
  group?: { id?: string; name?: string; modalityId?: string; level?: string; ageGroup?: string; roomId?: string; instructorId?: string; color?: string; scheduleColor?: string; capacity?: number; sourceType?: string; settings?: unknown; weekdays?: number[]; startsAt?: string; endsAt?: string; schedules?: Array<{ id?: string; weekday?: number; startsAt?: string; endsAt?: string; roomId?: string; instructorId?: string; ageGroup?: string; color?: string; capacity?: number | string; settings?: unknown }> };
...
export async function GET(request: Request) {
  try {
    ...
    const [groupsResult, schedulesResult, enrollmentsResult, studentsResult, rentalsResult, modalitiesResult, instructorsResult, roomsResult] = await Promise.all([
      admin.from("xpace_class_groups").select("id,name,modality,modality_id,class_level,instructor_id,color,capacity,source_type,settings,active").eq("tenant_company_id", company.id).order("name"),
      admin.from("xpace_class_schedules").select("id,class_group_id,weekday,starts_at,ends_at,room_name,room_id,instructor_id,age_group,color,capacity,settings,active").eq("tenant_company_id", company.id).eq("active", true).order("weekday").order("starts_at"),
    ...
    return NextResponse.json({ success: true, groups: (groupsResult.data ?? []).map((group) => ({ id: group.id, name: group.name, modality: group.modality ?? "", modalityId: group.modality_id ?? "", level: normalizeClassLevel(group.class_level), instructorName: group.instructor_id ? instructorNames.get(group.instructor_id) ?? "PROFESSOR ARQUIVADO" : "", color: "#7435D9", capacity: group.capacity, sourceType: group.source_type, settings: group.settings ?? {}, active: group.active, schedules: (schedulesResult.data ?? []).filter((schedule) => schedule.class_group_id === group.id).map((schedule) => ({ id: schedule.id, weekday: schedule.weekday, startsAt: schedule.starts_at.slice(0, 5), endsAt: schedule.ends_at.slice(0, 5), roomName: schedule.room_name ?? "", roomId: schedule.room_id ?? "", instructorId: schedule.instructor_id ?? "", instructorName: schedule.instructor_id ? instructorNames.get(schedule.instructor_id) ?? "PROFESSOR ARQUIVADO" : "", ageGroup: normalizeAgeGroup(schedule.age_group) || "ADULTO", color: normalizeColor(schedule.color), capacity: schedule.capacity ?? group.capacity, settings: schedule.settings ?? group.settings ?? {} })), students: enrollmentsByGroup.get(group.id) ?? [] })), modalities: (modalitiesResult.data ?? []).map((modality) => ({ id: modality.id, name: modality.name, instructorId: modality.instructor_id ?? "", instructorName: modality.instructor_id ? instructorNames.get(modality.instructor_id) ?? "PROFESSOR ARQUIVADO" : "", requiresInstructor: modality.requires_instructor })), instructors: (instructorsResult.data ?? []).map((instructor) => ({ id: instructor.id, fullName: instructor.full_name })), rooms: (roomsResult.data ?? []).map((room) => ({ id: room.id, name: room.name, capacity: room.capacity })), students: (studentsResult.data ?? []).map((student) => ({ id: student.id, name: student.full_name, mobile: student.mobile ?? "" })), rentals: (rentalsResult.data ?? []).map((rental) => ({ id: rental.id, roomName: rental.room_name, renterName: rental.renter_name, startsAt: rental.starts_at, endsAt: rental.ends_at, amountCents: rental.amount_cents, status: rental.status, note: rental.note ?? "" })) });
...
function normalizeGroup(input?: Body["group"]) { return { name: clean(input?.name), modalityId: clean(input?.modalityId), level: normalizeClassLevel(input?.level), ageGroup: normalizeAgeGroup(input?.ageGroup), color: "#7435D9", sourceType: input?.sourceType === "SERVICO" ? "SERVICO" : "CONTRATO" }; }
...
async function prepareScheduleRows(access: Awaited<ReturnType<typeof requireCompanyAccess>>, input: Body["group"] | undefined, modalityId: string, excludedGroupId = "") {
  const fallback = { roomId: clean(input?.roomId), instructorId: clean(input?.instructorId), ageGroup: normalizeAgeGroup(input?.ageGroup), color: normalizeColor(input?.scheduleColor), capacity: input?.capacity, settings: input?.settings };
  const raw = (input?.schedules ?? []).map((item) => ({ weekday: Number(item.weekday), startsAt: clean(item.startsAt), endsAt: clean(item.endsAt), roomId: clean(item.roomId) || fallback.roomId, instructorId: clean(item.instructorId) || fallback.instructorId, ageGroup: normalizeAgeGroup(item.ageGroup) || fallback.ageGroup, color: normalizeColor(item.color, fallback.color), capacity: item.capacity ?? fallback.capacity, settings: normalizeSettings(item.settings ?? fallback.settings) }));
  if (!raw.length || raw.some((slot) => !Number.isInteger(slot.weekday) || slot.weekday < 0 || slot.weekday > 6 || !isTime(slot.startsAt) || !isTime(slot.endsAt) || slot.endsAt <= slot.startsAt || !slot.roomId || !slot.instructorId || !slot.ageGroup)) throw new RequestError("CADA HORÁRIO PRECISA TER DIA, INÍCIO, DURAÇÃO, PÚBLICO, SALA E PROFESSOR.", 400);
  ...
  const rows = raw.map((slot) => {
    ...
    return { weekday: slot.weekday, starts_at: slot.startsAt, ends_at: slot.endsAt, room_name: room.name, room_id: room.id, instructor_id: slot.instructorId, age_group: slot.ageGroup, color: slot.color, capacity: requestedCapacity, settings: slot.settings, active: true };
  });
