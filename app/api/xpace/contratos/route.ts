...
type RequestBody = {
  ...
  plan?: { id?: string; name?: string; description?: string; billingInterval?: string; durationMonths?: number; amountCents?: number; active?: boolean };
  contract?: { id?: string; studentId?: string; planId?: string; startsOn?: string; amountCents?: number; status?: string; statusNote?: string };
};
...
export async function GET(request: Request) {
  try {
    ...
    const [plansResult, studentsResult, contractsResult] = await Promise.all([
      admin.from("xpace_membership_plans").select("id,name,description,billing_interval,duration_months,amount_cents,active,created_at").eq("tenant_company_id", company.id).order("active", { ascending: false }).order("name"),
      ...
      admin.from("xpace_student_contracts").select("id,contract_number,student_id,plan_id,plan_name_snapshot,billing_interval_snapshot,duration_months_snapshot,amount_cents,starts_on,ends_on,status,status_note,created_at").eq("tenant_company_id", company.id).order("created_at", { ascending: false }).limit(300),
    ]);
    ...
    return NextResponse.json({
      success: true,
      plans: (plansResult.data ?? []).map((plan) => ({ id: plan.id, name: plan.name, description: plan.description ?? "", billingInterval: plan.billing_interval, durationMonths: plan.duration_months, amountCents: plan.amount_cents, active: plan.active, createdAt: plan.created_at })),
...
async function createContract(access: Awaited<ReturnType<typeof requireCompanyAccess>>, input?: RequestBody["contract"]) {
  ...
  const [{ data: student, error: studentError }, { data: plan, error: planError }] = await Promise.all([
    ...
    access.admin.from("xpace_membership_plans").select("id,name,billing_interval,duration_months,amount_cents,active").eq("id", contract.planId).eq("tenant_company_id", access.company.id).maybeSingle(),
  ]);
  ...
  const amountCents = contract.amountCents === undefined ? plan.amount_cents : contract.amountCents;
  ...
  const { data: conflict, error: conflictError } = await access.admin.from("xpace_student_contracts").select("id").eq("tenant_company_id", access.company.id).eq("student_id", student.id).eq("plan_id", plan.id).in("status", ["AGENDADO", "ATIVO", "PAUSADO"]).lte("starts_on", endsOn).gte("ends_on", startsOn).maybeSingle();
  if (conflictError) throw conflictError;
  if (conflict) throw new RequestError("ESTE ALUNO JA POSSUI UM CONTRATO EM VIGOR PARA ESTE PLANO.", 409);
  const { data: saved, error: savedError } = await access.admin.from("xpace_student_contracts").insert({ tenant_company_id: access.company.id, student_id: student.id, plan_id: plan.id, plan_name_snapshot: plan.name, billing_interval_snapshot: plan.billing_interval, duration_months_snapshot: plan.duration_months, amount_cents: amountCents, starts_on: startsOn, ends_on: endsOn, status, created_by: access.user.id, updated_by: access.user.id }).select("id,contract_number").single();
...
async function setContractStatus(access: Awaited<ReturnType<typeof requireCompanyAccess>>, input?: RequestBody["contract"]) {
  ...
  const { error } = await access.admin.from("xpace_student_contracts").update({ status: contract.status, status_note: contract.statusNote || null, updated_by: access.user.id, updated_at: new Date().toISOString() }).eq("id", current.id).eq("tenant_company_id", access.company.id);
  if (error) throw error;
...
function normalizePlan(value?: RequestBody["plan"]) {
  return { name: value?.name?.trim().replace(/\s+/g, " ") ?? "", description: value?.description?.trim() ?? "", billingInterval: value?.billingInterval as BillingInterval, durationMonths: Number(value?.durationMonths), amountCents: Number(value?.amountCents) };
}
function normalizeContract(value?: RequestBody["contract"]) {
  return { id: value?.id?.trim() ?? "", studentId: value?.studentId?.trim() ?? "", planId: value?.planId?.trim() ?? "", startsOn: value?.startsOn?.trim() ?? "", amountCents: value?.amountCents === undefined ? undefined : Number(value.amountCents), status: value?.status as ContractStatus, statusNote: value?.statusNote?.trim() ?? "" };
}
