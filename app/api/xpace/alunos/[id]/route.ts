...
export async function GET(request: Request, context: Context) {
  try {
    ...
    const [chargesResult, activitiesResult, benefitsResult, profilesResult, rewardsResult, enrollmentsResult, salesResult, eventsResult, contractGroupsResult, groupSchedulesResult, modalitiesResult, photoResult] = await Promise.all([
      ...
      access.admin.from("xpace_contract_sales").select("id,sale_number,contract_id,sold_on,status,signature_required,signature_status,signature_due_on,signature_provider,signature_url,signed_document_url,signature_error,sent_for_signature_at,signed_at,cancelled_at,cancellation_reason,created_at,effective_at").eq("tenant_company_id", access.company.id).eq("student_id", id).order("sold_on", { ascending: false }).order("created_at", { ascending: false }).limit(240),
    ...
    const chargeIds = (chargesResult.data ?? []).map((charge) => charge.id);
    const { data: cancellations, error: cancellationsError } = chargeIds.length ? await access.admin.from("xpace_payment_cancellations").select("charge_id,status,last_error").eq("tenant_company_id", access.company.id).in("charge_id", chargeIds) : { data: [], error: null };
    if (cancellationsError) throw cancellationsError;
    const cancellationByCharge = new Map((cancellations ?? []).map((job) => [job.charge_id, job]));
    ...
    return NextResponse.json({
      ...
      contracts: (contracts ?? []).map((contract) => ({ id: contract.id, planId: contract.plan_id ?? "", contractNumber: contract.contract_number, planName: contract.plan_name_snapshot, billingInterval: contract.billing_interval_snapshot, durationMonths: contract.duration_months_snapshot, baseAmountCents: contract.base_amount_cents, amountCents: contract.amount_cents, benefitName: contract.benefit_name_snapshot ?? "", renewsAutomatically: contract.renews_automatically, startsOn: contract.starts_on, endsOn: contract.ends_on, status: contract.status, statusNote: contract.status_note ?? "", cancelEffectiveOn: contract.cancel_effective_on, paymentAccessBlocked: contract.payment_access_blocked, signatureAccessBlocked: contract.signature_access_blocked, createdAt: contract.created_at, sale: (() => { const sale = salesByContract.get(contract.id); return sale ? { id: sale.id, signatureRequired: sale.signature_required, signatureStatus: sale.signature_status, signatureDueOn: sale.signature_due_on, signatureUrl: sale.signature_url ?? "", signedDocumentUrl: sale.signed_document_url ?? "", signatureError: sale.signature_error ?? "", sentAt: sale.sent_for_signature_at, signedAt: sale.signed_at } : null; })(), groups: (groupsByContract.get(contract.id) ?? []).map((group) => ({ modalityName: modalitiesById.get(group.modalityId) ?? "MODALIDADE", schedules: schedulesByGroup.get(group.classGroupId) ?? [] })), events: (eventsResult.data ?? []).filter((event) => event.contract_id === contract.id).map((event) => ({ id: event.id, type: event.event_type, previousStatus: event.previous_status ?? "", nextStatus: event.next_status ?? "", note: event.note ?? "", createdAt: event.created_at })) })),
      sales: (salesResult.data ?? []).flatMap((sale) => {
        const contract = contractsById.get(sale.contract_id);
        if (!contract) return [];
        const enrollmentService = contract.enrollment_service_snapshot && typeof contract.enrollment_service_snapshot === "object" ? contract.enrollment_service_snapshot as { salePriceCents?: unknown } : null;
        const enrollmentFeeCents = Number(enrollmentService?.salePriceCents);
        const saleAmountCents = contract.amount_cents + (Number.isInteger(enrollmentFeeCents) && enrollmentFeeCents > 0 ? enrollmentFeeCents : 0);
        return [{ id: sale.id, saleNumber: sale.sale_number, contractId: sale.contract_id, contractNumber: contract.contract_number, planName: contract.plan_name_snapshot, soldAt: sale.sold_on ?? sale.created_at.slice(0, 10), status: sale.status, effectiveAt: sale.effective_at, contractStatus: contract.status, signatureRequired: sale.signature_required, signatureStatus: sale.signature_status, signatureDueOn: sale.signature_due_on, signatureProvider: sale.signature_provider ?? "", signatureUrl: sale.signature_url ?? "", signedDocumentUrl: sale.signed_document_url ?? "", signatureError: sale.signature_error ?? "", signedAt: sale.signed_at, cancelledAt: sale.cancelled_at, cancellationReason: sale.cancellation_reason ?? "", baseAmountCents: contract.base_amount_cents, amountCents: saleAmountCents, discountCents: Math.max(0, contract.base_amount_cents - contract.amount_cents) }];
      }),
      charges: charges.map((charge) => ({ id: charge.id, contractId: charge.contract_id, paymentMethod: contractsById.get(charge.contract_id)?.payment_method ?? "", competenceOn: charge.competence_on, dueOn: charge.due_on, baseAmountCents: charge.base_amount_cents, amountCents: charge.amount_cents, status: charge.status, paidAmountCents: charge.paid_amount_cents, paidAt: charge.paid_at, benefitName: charge.benefit_name_snapshot ?? "", providerPaymentId: charge.provider_payment_id ?? "", providerStatus: charge.provider_status ?? "", pixCopyPaste: charge.pix_copy_paste ?? "", pixQrCodeUrl: charge.pix_qr_code_url ?? "", providerError: charge.provider_error ?? "", cancellationStatus: cancellationByCharge.get(charge.id)?.status ?? "", cancellationError: cancellationByCharge.get(charge.id)?.last_error ?? "" })),
