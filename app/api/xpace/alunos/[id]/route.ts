...
export async function GET(request: Request, context: Context) {
  try {
    ...
    const [chargesResult, activitiesResult, benefitsResult, profilesResult, rewardsResult, enrollmentsResult, salesResult, photoResult] = await Promise.all([
      ...
      access.admin.from("xpace_contract_sales").select("id,sale_number,contract_id,sold_on,status,signature_required,signature_status,signature_provider,signature_url,signature_error,sent_for_signature_at,signed_at,cancelled_at,cancellation_reason,created_at").eq("tenant_company_id", access.company.id).eq("student_id", id).order("sold_on", { ascending: false }).order("created_at", { ascending: false }).limit(240),
      photoRequest,
    ]);
