...
export async function GET(request: Request) {
  try {
    ...
    const [clientsResult, profilesResult, activitiesResult, opportunitiesResult, quotesResult, sellersResult, peopleResult, settingsResult, membersResult] = await Promise.all([
      ...
      admin.from("crm_opportunities").select("id,client_id,representative_profile_id,quote_id,title,product_ficha_id,product_reference,stage,estimated_value,expected_close_date,lost_reason,created_at,updated_at").eq("tenant_company_id", company.id).order("updated_at", { ascending: false }).limit(5000),
