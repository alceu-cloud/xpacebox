...
export async function GET(request: Request) {
  try {
    ...
    const [clientsResult, profilesResult, activitiesResult, opportunitiesResult, quotesResult, salesOrdersResult, sellersResult, peopleResult, settingsResult, membersResult] = await Promise.all([
      ...
      admin.from("crm_activities").select("id,client_id,opportunity_id,agenda_kind,representative_profile_id,activity_type,outcome,subject,occurred_at,next_action_at").eq("tenant_company_id", company.id).order("occurred_at", { ascending: false }).limit(5000),
      admin.from("crm_opportunities").select("id,client_id,representative_profile_id,quote_id,title,product_ficha_id,product_reference,stage,estimated_value,expected_close_date,lost_reason,closed_at,created_at,updated_at").eq("tenant_company_id", company.id).order("updated_at", { ascending: false }).limit(5000),
      admin.from("quotes").select("id,client_id,representative_profile_id,seller_company_name,seller_company_slug,quote_number,grand_total,issue_date,valid_until,created_at,quote_items(item_number,ft_number,description,material,quantity,unit_price,total,snapshot)").eq("tenant_company_id", company.id).order("created_at", { ascending: false }).limit(5000),
      admin.from("sales_orders").select("id,client_id,representative_profile_id,crm_opportunity_id,sale_number,product_total,ipi_total,grand_total,contribution_total,mc_percent,ordered_at,created_at").eq("tenant_company_id", company.id).order("ordered_at", { ascending: false }).limit(5000),
    ...
    for (const result of [clientsResult, profilesResult, activitiesResult, opportunitiesResult, quotesResult, salesOrdersResult, sellersResult, peopleResult, settingsResult, membersResult]) {
    ...
    const salesOrders = (salesOrdersResult.data ?? []).filter((item) => isManager || item.representative_profile_id === user.id || visibleClientIds.has(item.client_id));
