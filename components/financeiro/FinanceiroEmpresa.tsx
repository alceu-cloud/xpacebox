  ...
  return (
    <section style={shellStyle}>
      ...
      <section style={panelStyle}>
        ...
        {message && <div style={message.includes("COM SUCESSO.") ? successMessageStyle : messageStyle}>{message}</div>}
      </section>
...
function QuoteForm({ kind, editing, form, items, clients, representatives, paymentConditions, clientFichas, selectedClientId, selectedFichaId, selectedClient, selectedFicha, updateForm, updateItem, selectClient, selectFicha, addItem, removeItem, onCancel, onSave }: any) {
  ...
  return <section style={formPanelStyle}><div style={formHeaderStyle}><div><span style={eyebrowStyle}>{kind === "DIRECT" ? "ORCAMENTO DIRETO" : "ORCAMENTO DE ENGENHARIA"}</span><h2 style={titleStyle}>MONTAR ORCAMENTO</h2></div><button type="button" onClick={onCancel} style={closeButtonStyle}>FECHAR</button></div>
    ...
    <div style={formGridStyle}>
      {kind === "DIRECT" ? <>
        <Field label="NOME DO CLIENTE" value={form.clientName} onChange={(value: string) => updateForm("clientName", value)} />
        <Field label="COMPRADOR" value={form.buyerName} onChange={(value: string) => updateForm("buyerName", value)} />
        <Field label="TELEFONE" value={form.phone} onChange={(value: string) => updateForm("phone", formatPhone(value))} />
        <Field label="E-MAIL" value={form.email} onChange={(value: string) => updateForm("email", value.toLowerCase())} lower />
        <Field label="CNPJ" value={form.cnpj} onChange={(value: string) => updateForm("cnpj", formatCnpj(value))} />
      </> : null}
      <Field label="EMPRESA VENDEDORA" value={form.company} onChange={(value: string) => updateForm("company", value)} />
      <Field label="REPRESENTANTE" value={form.representative} onChange={(value: string) => updateForm("representative", value)} options={representatives.map((item: RepresentativeOption) => ({ value: item.name, label: item.name }))} placeholder="SELECIONE O REPRESENTANTE" />
      <Field label="CONDICAO DE PAGAMENTO" value={form.paymentTerms} onChange={(value: string) => updateForm("paymentTerms", value)} options={paymentConditions.map((item: PaymentCondition) => ({ value: item.name, label: item.name }))} placeholder="SELECIONE A CONDICAO" />
      {kind === "DIRECT" ? <Field label="FRETE" value={form.freight} onChange={(value: string) => updateForm("freight", value)} options={[{ value: "CIF", label: "CIF - REMETENTE" }, { value: "FOB", label: "FOB - RETIRADA / DESTINATARIO" }, { value: "SEM_FRETE", label: "SEM FRETE" }]} /> : null}
      <Field label="DATA DE ENTREGA" value={form.deliveryDate} onChange={(value: string) => updateForm("deliveryDate", value)} type="date" />
      <Field label="VALIDADE DO ORCAMENTO (AUTOMATICA)" value={form.validUntil} onChange={() => undefined} type="date" readOnly />
    ...
    {items.map((item: QuoteItem, index: number) => <div key={index} style={itemCardStyle}>
      ...
      <div style={itemGridStyle}>
        <Field label="FICHA TECNICA" value={item.ftNumber} onChange={(value: string) => updateItem(index, "ftNumber", value)} />
        <Field label="DESCRICAO" value={item.description} onChange={(value: string) => updateItem(index, "description", value)} />
        {kind === "DIRECT" ? <>
          <Field label="TIPO DE CAIXA" value={item.boxType} onChange={(value: string) => updateItem(index, "boxType", value)} />
          <Field label="MATERIAL" value={item.material} onChange={(value: string) => updateItem(index, "material", value)} />
          <Field label="COMPRIMENTO (MM)" value={String(item.length || "")} onChange={(value: string) => updateItem(index, "length", value)} type="number" />
          <Field label="LARGURA (MM)" value={String(item.width || "")} onChange={(value: string) => updateItem(index, "width", value)} type="number" />
          <Field label="ALTURA (MM)" value={String(item.height || "")} onChange={(value: string) => updateItem(index, "height", value)} type="number" />
          <Field label="AREA (M2)" value={String(item.area || "")} onChange={(value: string) => updateItem(index, "area", value)} type="number" />
          <Field label="QUALIDADE" value={item.quality} onChange={(value: string) => updateItem(index, "quality", value)} />
        </> : null}
        <Field label="QUANTIDADE" value={String(item.quantity || "")} onChange={(value: string) => updateItem(index, "quantity", value)} type="number" />
        <Field label="VALOR UNITARIO" value={String(item.unitPrice || "")} onChange={(value: string) => updateItem(index, "unitPrice", value)} currency />
        {kind === "DIRECT" ? <Field label="IPI (%)" value={String(appliesIpi ? item.ipiPercent || "" : 0)} onChange={(value: string) => updateItem(index, "ipiPercent", value)} type="number" readOnly={!appliesIpi} /> : null}
      </div>
    </div>)}
...
function Field({ label, value, onChange, type = "text", lower = false, options, placeholder, readOnly = false, currency = false }: { label: string; value: string; onChange: (value: string) => void; type?: string; lower?: boolean; options?: Array<{ value: string; label: string }>; placeholder?: string; readOnly?: boolean; currency?: boolean }) {
  return <label style={labelStyle}>{label}{options ? <SearchableSelect value={value} onChange={onChange} options={options} placeholder={placeholder ?? "SELECIONE"} inputStyle={inputStyle} ariaLabel={label} /> : currency ? <CurrencyInput value={value} onValueChange={(nextValue) => onChange(nextValue === null ? "" : String(nextValue))} readOnly={readOnly} style={inputStyle} /> : <input type={type} value={value} onChange={(event) => onChange(event.target.value)} readOnly={readOnly} style={{ ...inputStyle, textTransform: lower ? "none" : "uppercase", ...(readOnly ? { background: "#f2f4f7", color: "#667085", cursor: "default" } : {}) }} />}</label>;
}
...
const successMessageStyle = { ...messageStyle, background: "rgba(0,156,75,.10)", color: "#087a3d", border: "1px solid rgba(0,156,75,.28)" };
...
const quoteListTabStyle = { border: "1px solid rgba(111,50,210,.24)", color: "#667085", background: "#fff", cursor: "pointer" , ...ui.tab };
...
const quoteDateFilterLabelStyle = { display: "grid", gap: 5, color: "#667085", ...ui.label };
const quoteDateInputStyle = { border: "1px solid rgba(111,50,210,.24)", background: "#fff", color: "#344054", outline: "none" , ...ui.field };
...
const quoteRowStyle = { display: "flex", justifyContent: "space-between", alignItems: "center", gap: 18, ...ui.frame, flexWrap: "wrap" as const };
const quoteNumberStyle = { display: "block", color: "#6f32d2", fontSize: 16, fontWeight: 900 };
...
const opportunityLinkItemStyle = { display: "flex", justifyContent: "space-between", alignItems: "center", gap: 14, padding: "14px 16px", border: "1px solid rgba(111,50,210,.16)", borderRadius: 10, background: "#fff" , flexWrap: "wrap" as const };
