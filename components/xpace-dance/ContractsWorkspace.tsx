...
type Plan = { id: string; name: string; description: string; billingInterval: string; durationMonths: number; amountCents: number; active: boolean; createdAt: string };
type Student = { id: string; personNumber: number; name: string; mobile: string };
type Contract = { id: string; contractNumber: number; studentId: string; studentName: string; studentMobile: string; planId: string | null; planName: string; billingInterval: string; durationMonths: number; amountCents: number; startsOn: string; endsOn: string; status: Status; statusNote: string; createdAt: string };
...
export default function ContractsWorkspace() {
  ...
  const [planDraft, setPlanDraft] = useState({ name: "", description: "", billingInterval: "MENSAL", durationMonths: "1", amount: "" });
  const [contractDraft, setContractDraft] = useState({ studentId: "", planId: "", startsOn: todayIso(), amount: "" });
  ...
  function startPlan() { setPlanDraft({ name: "", description: "", billingInterval: "MENSAL", durationMonths: "1", amount: "" }); setNotice(""); setView("NEW_PLAN"); }
  function startContract() { setContractDraft({ studentId: students[0]?.id ?? "", planId: activePlans[0]?.id ?? "", startsOn: todayIso(), amount: activePlans[0] ? centsToInput(activePlans[0].amountCents) : "" }); setNotice(""); setView("NEW_CONTRACT"); }
  ...
  if (view === "NEW_CONTRACT") return <section className="xd-contracts"><ModuleHeader eyebrow="MATRÍCULA E VIGÊNCIA" title="NOVO CONTRATO." copy="Selecione o aluno e o plano. As condições são copiadas para o contrato e ficam preservadas mesmo quando o catálogo for atualizado." back={() => setView("CONTRACTS")} />
    {!students.length || !activePlans.length ? <EmptySetup students={students.length} plans={activePlans.length} onPlans={() => setView("PLANS")} onStudents={() => setView("CONTRACTS")} /> : <form className="xd-contract-form" onSubmit={createContract}>
      <div className="xd-contract-form-grid xd-contract-form-grid--contract">
        ...
        <label>PLANO<select value={contractDraft.planId} onChange={(event) => { const plan = activePlans.find((item) => item.id === event.target.value); setContractDraft({ ...contractDraft, planId: event.target.value, amount: plan ? centsToInput(plan.amountCents) : "" }); }}>{activePlans.map((plan) => <option key={plan.id} value={plan.id}>{plan.name} · {formatCurrency(plan.amountCents)}</option>)}</select></label>
      ...
      {selectedPlan ? <div className="xd-contract-preview"><CalendarClock size={19} /><span><small>VIGÊNCIA CALCULADA</small><strong>{selectedPlan.durationMonths} {selectedPlan.durationMonths === 1 ? "MÊS" : "MESES"} · {selectedPlan.billingInterval}</strong></span><b>{formatCurrency(inputToCents(contractDraft.amount))}</b></div> : null}
