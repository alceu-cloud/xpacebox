...
export default function CommunityWorkspace({ onBack }: { onBack: () => void }) {
  ...
  if (mode === "FORM") return <section className="xd-community">
    ...
    <form className="xd-person-form" onSubmit={submit}>
      ...
      <section className="xd-form-block"><div><span>03</span><h2>ENDEREÇO</h2><p>Informações completas para o relacionamento com a família.</p></div><div className="xd-fields xd-fields--address"><Field label="CEP" value={student.postalCode} onChange={(value) => updateDraft(setStudent, student, "postalCode", value.replace(/\D/g, "").slice(0, 8))} inputMode="numeric" /><Field label="RUA / AVENIDA" value={student.street} onChange={(value) => updateDraft(setStudent, student, "street", value)} /><Field label="NÚMERO" value={student.streetNumber} onChange={(value) => updateDraft(setStudent, student, "streetNumber", value)} /><Field label="COMPLEMENTO" value={student.complement} onChange={(value) => updateDraft(setStudent, student, "complement", value)} /><Field label="BAIRRO" value={student.district} onChange={(value) => updateDraft(setStudent, student, "district", value)} /><Field label="CIDADE" value={student.city} onChange={(value) => updateDraft(setStudent, student, "city", value)} /><Field label="UF" value={student.state} onChange={(value) => updateDraft(setStudent, student, "state", value.toUpperCase().slice(0, 2))} /></div></section>
  ...
  return <section className="xd-community">
    <header className="xd-community-header"><button type="button" className="xd-return" onClick={onBack}><ArrowLeft size={17} /> PAINEL XPACE</button><span>COMUNIDADE</span></header>
    ...
    <div className="xd-people-list">{loading ? <p>CARREGANDO COMUNIDADE...</p> : visiblePeople.length ? visiblePeople.map((person) => <article key={person.id} className="xd-person-row"><div className="xd-person-icon"><UsersRound size={20} /></div><div><strong>{person.name}</strong><small>{person.isStudent ? "ALUNO" : "RESPONSÁVEL"} · {person.cpfMasked} · {person.mobile || "SEM CELULAR"}</small></div><div className="xd-person-meta"><span>{person.city ? `${person.city}/${person.state}` : "SEM ENDEREÇO"}</span>{person.isStudent && person.guardians.length ? <small>RESP.: {person.guardians.map((guardian) => guardian.name).join(", ")}</small> : null}</div><i>{String(person.personNumber).padStart(4, "0")}</i></article>) : <p className="xd-empty-community">NENHUMA PESSOA ENCONTRADA. CADASTRE O PRIMEIRO ALUNO.</p>}</div>
  </section>;
}

function Field({ label, value, onChange, type = "text", inputMode }: { label: string; value: string; onChange: (value: string) => void; type?: string; inputMode?: "numeric" | "tel" }) { return <label>{label}<input type={type} inputMode={inputMode} value={value} onChange={(event) => onChange(event.target.value)} /></label>; }
