...
type Person = { id: string; personNumber: number; name: string; photoUrl: string; cpfMasked: string; mobile: string; birthDate: string; email: string; gender: string; isStudent: boolean; whatsappOptIn: boolean; city: string; state: string; guardians: Array<{ id: string; name: string; cpfMasked: string; mobile: string }> };
...
export default function CommunityWorkspace({ onOpenProfile }: { onOpenProfile: (id: string) => void }) {
  ...
  return <section className="xd-community">
    ...
    <div className="xd-people-list">{loading ? <p>CARREGANDO COMUNIDADE...</p> : visiblePeople.length ? visiblePeople.map((person) => <article key={person.id} className="xd-person-row"><div className="xd-person-icon">{person.photoUrl ? <img src={person.photoUrl} alt="" loading="lazy" /> : <UsersRound size={20} />}</div><div><button type="button" className="xd-person-open" disabled={!person.isStudent} onDoubleClick={() => person.isStudent && onOpenProfile(person.id)} onClick={() => person.isStudent && onOpenProfile(person.id)} title={person.isStudent ? "Abrir perfil do aluno" : "O perfil geral está disponível apenas para alunos"}><span className="xd-person-name-line"><strong>{person.name}</strong><i>{String(person.personNumber).padStart(4, "0")}</i></span></button><small>{person.isStudent ? "ALUNO" : "RESPONSÁVEL"} · {person.cpfMasked} · {person.mobile || "SEM CELULAR"}</small></div><div className="xd-person-meta"><span>{person.city ? `${person.city}/${person.state}` : "SEM ENDEREÇO"}</span>{person.isStudent && person.guardians.length ? <small>RESP.: {person.guardians.map((guardian) => guardian.name).join(", ")}</small> : null}</div>{person.isStudent ? <button type="button" className="xd-person-profile-action" onClick={() => onOpenProfile(person.id)}>ABRIR</button> : null}</article>) : <p className="xd-empty-community">NENHUMA PESSOA ENCONTRADA. CADASTRE O PRIMEIRO ALUNO.</p>}</div>
  </section>;
}
