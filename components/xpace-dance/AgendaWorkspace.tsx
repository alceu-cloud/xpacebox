...
import { CalendarDays, CheckCircle2, ChevronLeft, ChevronRight, Clock3, DoorOpen, LayoutList, Plus, Save, Settings2, UsersRound, XCircle } from "lucide-react";
...
type ScheduleDraft = { id?: string; weekday: number; startsAt: string; endsAt: string; roomId: string; instructorId: string; level: ClassLevel; ageGroup: AgeGroup; ageGroups: AgeGroup[]; color: string; capacity: string; settings: GradeSettings };
type Group = { id: string; name: string; modality: string; modalityId: string; level: ClassLevel; instructorName: string; color: string; capacity: number | null; sourceType: "CONTRATO" | "SERVICO"; settings: GradeSettings; active: boolean; schedules: Array<{ id: string; weekday: number; startsAt: string; endsAt: string; roomName: string; roomId: string; instructorName: string; instructorId: string; level: ClassLevel; ageGroup: AgeGroup; ageGroups: AgeGroup[]; color: string; capacity: number | null; settings: GradeSettings }>; students: Array<{ id: string; studentId: string; studentName: string; mobile: string; startsOn: string; endsOn: string | null }> };
type TrialAppointment = { id: string; leadId: string; leadName: string; leadMobile: string; classScheduleId: string; scheduledOn: string; attendanceStatus: "AGENDADO" | "COMPARECEU" | "FALTOU" | "CANCELADO" | "NAO_INFORMADO" };
type Workspace = { groups: Group[]; trialAppointments: TrialAppointment[]; modalities: Array<{ id: string; name: string; instructorId: string; instructorName: string; requiresInstructor: boolean }>; instructors: Array<{ id: string; fullName: string }>; rooms: Array<{ id: string; name: string; capacity: number | null }>; students: Array<{ id: string; name: string; mobile: string }>; rentals: Array<{ id: string; roomName: string; renterName: string; startsAt: string; endsAt: string; amountCents: number; status: string; note: string }> };
type CalendarSelection = { group: Group; schedule: Group["schedules"][number]; scheduledOn: string };
type ClassDraft = { name: string; modalityId: string; level: ClassLevel; ageGroup: AgeGroup; ageGroups: AgeGroup[]; roomId: string; instructorId: string; color: string; scheduleColor: string; capacity: string; sourceType: "CONTRATO" | "SERVICO"; weekdays: number[]; startsAt: string; durationMinutes: string; schedules: ScheduleDraft[]; settings: GradeSettings };
...
const ageGroupLabel = (ageGroup: AgeGroup) => ageGroups.find((item) => item.value === ageGroup)?.label ?? "ADULT (18+)";
const normalizeAgeGroups = (values: AgeGroup[]) => ageGroups.map((item) => item.value).filter((value) => values.includes(value));
const ageGroupsLabel = (values: AgeGroup[]) => normalizeAgeGroups(values).map(ageGroupLabel).join(" · ") || "ADULT (18+)";
...
const emptyScheduleDraft = (): ScheduleDraft => ({ weekday: 1, startsAt: "19:00", endsAt: "20:00", roomId: "", instructorId: "", level: "INICIANTE", ageGroup: "ADULTO", ageGroups: ["ADULTO"], color: "#7435D9", capacity: "", settings: emptyGradeSettings() });
const emptyClassDraft = (): ClassDraft => ({ name: "", modalityId: "", level: "INICIANTE", ageGroup: "ADULTO", ageGroups: ["ADULTO"], roomId: "", instructorId: "", color: "#7435D9", scheduleColor: "#7435D9", capacity: "", sourceType: "CONTRATO", weekdays: [], startsAt: "19:00", durationMinutes: "60", schedules: [], settings: emptyGradeSettings() });

function AgeGroupPicker({ value, onChange }: { value: AgeGroup[]; onChange: (value: AgeGroup[]) => void }) {
  const selected = normalizeAgeGroups(value);
  function toggle(ageGroup: AgeGroup) {
    const next = selected.includes(ageGroup) ? selected.filter((item) => item !== ageGroup) : [...selected, ageGroup];
    if (!next.length) return;
    onChange(normalizeAgeGroups(next));
  }
  return <fieldset className="xd-age-groups-picker"><legend>PÚBLICO DESTE HORÁRIO</legend><div>{ageGroups.map((ageGroup) => <label key={ageGroup.value}><input type="checkbox" checked={selected.includes(ageGroup.value)} onChange={() => toggle(ageGroup.value)} /><span>{ageGroup.label}</span></label>)}</div><small aria-live="polite">{ageGroupsLabel(selected)}</small></fieldset>;
}

export default function AgendaWorkspace() {
  const [view, setView] = useState<View>("AGENDA");
  const [workspace, setWorkspace] = useState<Workspace>({ groups: [], trialAppointments: [], modalities: [], instructors: [], rooms: [], students: [], rentals: [] });
  const [loading, setLoading] = useState(true);
  const [saving, setSaving] = useState(false);
  const [notice, setNotice] = useState("");
  const [month, setMonth] = useState(() => new Date(new Date().getFullYear(), new Date().getMonth(), 1));
  const [selectedDay, setSelectedDay] = useState("");
  const [selectedClass, setSelectedClass] = useState<CalendarSelection | null>(null);
  ...
  useEffect(() => { setSelectedDay(""); setSelectedClass(null); void loadWorkspace(); }, [month]);
  ...
  async function loadWorkspace() { const range = calendarRange(month); setLoading(true); try { setWorkspace(await request<Workspace>(`/api/xpace/agenda?from=${range.from}&to=${range.to}`)); } catch (error) { setNotice(error instanceof Error ? error.message : "NÃO FOI POSSÍVEL CARREGAR A AGENDA."); } finally { setLoading(false); } }
  async function submitClass(event: FormEvent): Promise<boolean> {
    event.preventDefault();
    const welcomeVideo = classDraft.settings.leadWelcomeVideoUrl?.trim();
    const hasInvalidSchedule = classDraft.schedules.some((schedule) => !schedule.roomId || !schedule.instructorId || !schedule.ageGroups.length || !schedule.startsAt || !schedule.endsAt || schedule.endsAt <= schedule.startsAt);
    if (!classDraft.name.trim() || !classDraft.modalityId) { setNotice("PREENCHA O NOME DA TURMA E SELECIONE A MODALIDADE ANTES DE SALVAR."); return false; }
    if (!classDraft.schedules.length) { setNotice("ADICIONE AO MENOS UM HORÁRIO CONFIGURADO PARA A GRADE."); return false; }
    if (hasInvalidSchedule) { setNotice("CADA HORÁRIO PRECISA TER DIA, INÍCIO, FIM, SALA E PROFESSOR."); return false; }
    if (classDraft.settings.allowLeads && welcomeVideo) {
      try { new URL(welcomeVideo); } catch { setNotice("O LINK DO VÍDEO DE BOAS-VINDAS NÃO É VÁLIDO. COLE O LINK COMPLETO, COMEÇANDO POR HTTPS://."); return false; }
    }
    setSaving(true); setNotice("");
    try { await request("/api/xpace/agenda", { method: "POST", body: JSON.stringify({ action: "CREATE_CLASS", group: classDraft }) }); setClassDraft(emptyClassDraft()); await loadWorkspace(); setNotice("GRADE CADASTRADA E INSERIDA NA AGENDA."); return true; } catch (error) { setNotice(error instanceof Error ? error.message : "NÃO FOI POSSÍVEL CADASTRAR A GRADE."); return false; } finally { setSaving(false); }
  }
  ...
  async function deleteGrade(group: Group) { if (!window.confirm(`EXCLUIR A GRADE ${group.name}?\n\nOS HORÁRIOS DELA SERÃO REMOVIDOS. A EXCLUSÃO É BLOQUEADA QUANDO HÁ ALUNOS MATRICULADOS OU CONTRATOS VINCULADOS.`)) return; setSaving(true); setNotice(""); try { await request("/api/xpace/agenda", { method: "DELETE", body: JSON.stringify({ action: "DELETE_GRADE", group: { id: group.id } }) }); if (selectedClass?.group.id === group.id) setSelectedClass(null); await loadWorkspace(); setNotice("GRADE EXCLUÍDA. VOCÊ JÁ PODE CADASTRÁ-LA NOVAMENTE."); } catch (error) { setNotice(error instanceof Error ? error.message : "NÃO FOI POSSÍVEL EXCLUIR A GRADE."); } finally { setSaving(false); } }
  ...
  async function markTrialAttendance(trial: TrialAppointment, attendanceStatus: "COMPARECEU" | "FALTOU") { setSaving(true); setNotice(""); try { await request("/api/xpace/agenda", { method: "POST", body: JSON.stringify({ action: "UPDATE_TRIAL_ATTENDANCE", trialAttendance: { appointmentId: trial.id, classScheduleId: trial.classScheduleId, scheduledOn: trial.scheduledOn, attendanceStatus } }) }); await loadWorkspace(); setNotice(`${trial.leadName}: ${attendanceStatus === "COMPARECEU" ? "COMPARECIMENTO" : "FALTA"} REGISTRADO(A).`); } catch (error) { setNotice(error instanceof Error ? error.message : "NÃO FOI POSSÍVEL REGISTRAR A PRESENÇA."); } finally { setSaving(false); } }

  return <section className="xd-agenda"><header className="xd-agenda-title"><div><span>AGENDA · OPERAÇÃO DE AULAS</span><h1>AGENDA E GRADES.</h1><p>Uma agenda mensal alimentada pelas grades cadastradas, com alunos vinculados em cada turma.</p></div></header><nav className="xd-agenda-nav">{([{ id: "AGENDA", icon: CalendarDays, label: "AGENDA" }, { id: "LOCACAO", icon: DoorOpen, label: "LOCAÇÃO" }, { id: "GRADES", icon: LayoutList, label: "GRADES DE HORÁRIOS" }, { id: "OCUPACAO", icon: Clock3, label: "OCUPAÇÃO" }] as Array<{ id: View; icon: typeof CalendarDays; label: string }>).map(({ id, icon: Icon, label }) => <button key={id} type="button" className={view === id ? "is-active" : ""} onClick={() => setView(id)}><Icon size={17} /> {label}</button>)}</nav>{notice ? <p className="xd-feedback">{notice}</p> : null}{loading ? <p className="xd-profile-loading">CARREGANDO AGENDA...</p> : view === "AGENDA" ? <Calendar month={month} setMonth={setMonth} groups={workspace.groups} trialAppointments={workspace.trialAppointments} selectedDay={selectedDay} selected={selectedClass} onSelectDay={(day) => { setSelectedDay(day); setSelectedClass(null); }} onSelect={(selection) => { setSelectedDay(selection.scheduledOn); setSelectedClass(selection); }} saving={saving} onMarkTrialAttendance={markTrialAttendance} /> : null}{view === "GRADES" ? <Grades groups={workspace.groups} modalities={workspace.modalities} instructors={workspace.instructors} rooms={workspace.rooms} students={workspace.students} classDraft={classDraft} setClassDraft={setClassDraft} enrollment={enrollment} setEnrollment={setEnrollment} saving={saving} notice={notice} onNoticeClear={() => setNotice("")} onClass={submitClass} onEnrollment={submitEnrollment} onSettings={setSettingsGroup} onDelete={deleteGrade} /> : null}{view === "LOCACAO" ? <Rentals rentals={workspace.rentals} draft={rental} setDraft={setRental} saving={saving} onSubmit={submitRental} /> : null}{view === "OCUPACAO" ? <Occupation groups={workspace.groups} rooms={workspace.rooms} rentals={workspace.rentals} /> : null}{settingsGroup ? <GradeSettingsDialog group={settingsGroup} modalities={workspace.modalities} instructors={workspace.instructors} rooms={workspace.rooms} saving={saving} onClose={() => setSettingsGroup(null)} onSave={(draft) => saveGrade(settingsGroup.id, draft)} /> : null}</section>;
}

function Calendar({ month, setMonth, groups, trialAppointments, selectedDay, selected, onSelectDay, onSelect, saving, onMarkTrialAttendance }: { month: Date; setMonth: (value: Date) => void; groups: Group[]; trialAppointments: TrialAppointment[]; selectedDay: string; selected: CalendarSelection | null; onSelectDay: (day: string) => void; onSelect: (selection: CalendarSelection) => void; saving: boolean; onMarkTrialAttendance: (trial: TrialAppointment, attendanceStatus: "COMPARECEU" | "FALTOU") => Promise<void> }) {
  const days = useMemo(() => monthDays(month), [month]);
  const eventsByDate = useMemo(() => new Map(days.map((day) => [isoDay(day), groups.flatMap((group) => group.schedules.filter((schedule) => schedule.weekday === day.getDay()).map((schedule) => ({ group, schedule, scheduledOn: isoDay(day) })))])), [days, groups]);
  const dayEvents = selectedDay ? eventsByDate.get(selectedDay) ?? [] : [];
  const selectedStudents = selected ? selected.group.students.filter((student) => student.startsOn <= selected.scheduledOn && (!student.endsOn || student.endsOn >= selected.scheduledOn)) : [];
  const selectedTrials = selected ? trialAppointments.filter((trial) => trial.classScheduleId === selected.schedule.id && trial.scheduledOn === selected.scheduledOn).sort((left, right) => naturalCompare(left.leadName, right.leadName)) : [];
  const detail = selected ? <><header><div><span style={{ background: selected.schedule.color }} /><small>AULA SELECIONADA · {formatDay(selected.scheduledOn)}</small><h2>{selected.group.name}</h2><p>{selected.group.modality || "SEM MODALIDADE"} · {selected.schedule.startsAt}–{selected.schedule.endsAt}</p></div><button type="button" title="Voltar para as aulas do dia" onClick={() => onSelectDay(selected.scheduledOn)}>×</button></header><div className="xd-detail-schedules"><span>{weekday[selected.schedule.weekday]} · {classLevelLabel(selected.schedule.level)}{selected.schedule.roomName ? ` · ${selected.schedule.roomName}` : ""}{selected.schedule.instructorName ? ` · ${selected.schedule.instructorName}` : ""}</span></div><div className="xd-class-roster"><strong><UsersRound size={16} /> CHAMADA ({selectedStudents.length + selectedTrials.length}{selected.schedule.capacity ? `/${selected.schedule.capacity}` : ""})</strong>{selectedStudents.map((student) => <article key={student.id} className="is-student"><div><b>ALUNO</b><span>{student.studentName}</span><small>{student.mobile || "SEM CELULAR"}</small></div></article>)}{selectedTrials.map((trial) => <article key={trial.id} className="is-lead"><div><b>LEAD</b><span>{trial.leadName}</span><small>{trial.leadMobile || "SEM CELULAR"} · {attendanceLabel(trial.attendanceStatus)}</small></div><footer><button type="button" className={trial.attendanceStatus === "COMPARECEU" ? "is-present" : ""} title="Marcar compareceu" disabled={saving} onClick={() => { void onMarkTrialAttendance(trial, "COMPARECEU"); }}><CheckCircle2 size={14} /> COMPARECEU</button><button type="button" className={trial.attendanceStatus === "FALTOU" ? "is-absent" : ""} title="Marcar faltou" disabled={saving} onClick={() => { void onMarkTrialAttendance(trial, "FALTOU"); }}><XCircle size={14} /> FALTOU</button></footer></article>)}{!selectedStudents.length && !selectedTrials.length ? <p>NENHUM ALUNO OU LEAD NESTA AULA.</p> : null}</div></> : selectedDay ? <><header><div><small>AULAS DO DIA · {formatDay(selectedDay)}</small><h2>{dayEvents.length} AULA(S)</h2><p>Escolha uma aula para abrir a chamada.</p></div><button type="button" title="Fechar dia" onClick={() => onSelectDay("")}>×</button></header><div className="xd-day-class-list">{dayEvents.map((event) => <button type="button" key={`${event.group.id}-${event.schedule.id}`} onClick={() => onSelect(event)}><i style={{ background: event.schedule.color }} /><div><strong>{event.schedule.startsAt}–{event.schedule.endsAt} · {event.group.name}</strong><small>{event.schedule.roomName || "SALA A DEFINIR"} · {event.schedule.instructorName || "PROFESSOR A DEFINIR"}</small></div></button>)}{!dayEvents.length ? <p>NENHUMA AULA CADASTRADA NESTA DATA.</p> : null}</div></> : <div className="xd-calendar-hint"><CalendarDays size={24} /><strong>ESCOLHA UMA DATA</strong><p>Clique no número de um dia para ver todas as aulas. Depois abra a aula desejada para fazer a chamada.</p></div>;
  return <div className="xd-calendar-layout"><section className="xd-calendar"><header><div className="xd-calendar-month"><button type="button" title="Mês anterior" onClick={() => setMonth(addMonths(month, -1))}><ChevronLeft size={19} /></button><div><small>VISUALIZAÇÃO MENSAL</small><strong>{month.toLocaleDateString("pt-BR", { month: "long", year: "numeric" }).toUpperCase()}</strong></div><button type="button" title="Próximo mês" onClick={() => setMonth(addMonths(month, 1))}><ChevronRight size={19} /></button></div><button type="button" className="xd-secondary" onClick={() => setMonth(new Date(new Date().getFullYear(), new Date().getMonth(), 1))}>HOJE</button></header><div className="xd-calendar-weekdays">{["SEG", "TER", "QUA", "QUI", "SEX", "SÁB", "DOM"].map((item) => <span key={item}>{item}</span>)}</div><div className="xd-calendar-grid">{calendarCells(month).map((day) => { const date = isoDay(day); const events = eventsByDate.get(date) ?? []; const outside = day.getMonth() !== month.getMonth(); return <article key={date} className={`${outside ? "is-outside " : ""}${selectedDay === date ? "is-selected" : ""}`}><button type="button" className="xd-calendar-date" onClick={() => onSelectDay(date)}>{day.getDate()}</button><div>{events.map((event) => <button type="button" key={`${event.group.id}-${event.schedule.id}-${event.scheduledOn}`} onClick={() => onSelect(event)} title={`${event.group.name} · ${event.schedule.startsAt} às ${event.schedule.endsAt} · ${classLevelLabel(event.schedule.level)}`}><i style={{ background: event.schedule.color }} /><span>{event.schedule.startsAt}</span><strong>{event.group.name}</strong></button>)}</div></article>; })}</div></section><aside className="xd-calendar-detail">{detail}</aside></div>;
}
  ...
  return <div className="xd-agenda-stack"><header className="xd-grade-list"><span><LayoutList size={18} /> GRADES CADASTRADAS</span><small>{groups.length} GRADE(S)</small><button type="button" className="xd-primary" onClick={() => setCreating(true)}><Plus size={16} /> NOVA GRADE</button></header>{creating ? <div className="xd-contract-overlay" role="presentation"><form className="xd-settings-dialog xd-modalities-picker xd-grade-create-dialog" onSubmit={(event) => { void onClass(event).then((saved) => { if (saved) setCreating(false); }); }} role="dialog" aria-modal="true" aria-labelledby="xd-new-grade-title"><header><span><Plus size={20} /></span><h2 id="xd-new-grade-title">NOVA GRADE</h2><button type="button" onClick={() => setCreating(false)} aria-label="Fechar">×</button></header><div className="xd-agenda-two-columns" tabIndex={0} role="region" aria-label="Dados e configurações da grade">
    <div className="xd-agenda-form">
      <div className="xd-agenda-fields">
        ...
        <label>PÚBLICO DESTE HORÁRIO<select value={classDraft.ageGroup} onChange={(event) => { const ageGroup = event.target.value as AgeGroup; setClassDraft({ ...classDraft, ageGroup, ageGroups: [ageGroup] }); }}>{ageGroups.map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}</select></label>
...
function addSchedulesToDraft(draft: ClassDraft, selection: ScheduleSelection, endsAt: string) {
  const scheduleKey = (schedule: Pick<ScheduleDraft, "weekday" | "startsAt" | "endsAt" | "roomId" | "instructorId" | "level" | "ageGroups">) => [schedule.weekday, schedule.startsAt, schedule.endsAt, schedule.roomId, schedule.instructorId, schedule.level, normalizeAgeGroups(schedule.ageGroups).join(",")].join(":");
  const existing = new Set(draft.schedules.map(scheduleKey));
  const additions = [...new Set(selection.weekdays)]
    .filter((day) => !existing.has(scheduleKey({ weekday: day, startsAt: selection.startsAt, endsAt, roomId: draft.roomId, instructorId: draft.instructorId, level: draft.level, ageGroups: draft.ageGroups })))
    .map((weekday): ScheduleDraft => ({
      ...
      level: draft.level,
      ageGroup: draft.ageGroups[0] ?? "ADULTO",
      ageGroups: normalizeAgeGroups(draft.ageGroups),
      color: draft.scheduleColor,
...
function Grades({ groups, modalities, instructors, rooms, students, classDraft, setClassDraft, enrollment, setEnrollment, saving, notice, onNoticeClear, onClass, onEnrollment, onSettings, onDelete }: { groups: Group[]; modalities: Workspace["modalities"]; instructors: Workspace["instructors"]; rooms: Workspace["rooms"]; students: Workspace["students"]; classDraft: ClassDraft; setClassDraft: (value: ClassDraft) => void; enrollment: { classGroupId: string; studentId: string; startsOn: string }; setEnrollment: (value: { classGroupId: string; studentId: string; startsOn: string }) => void; saving: boolean; notice: string; onNoticeClear: () => void; onClass: (event: FormEvent) => Promise<boolean>; onEnrollment: (event: FormEvent) => void; onSettings: (group: Group) => void; onDelete: (group: Group) => Promise<void> }) {
  const [creating, setCreating] = useState(false);
    ...
    {creating ? <div className="xd-contract-overlay" role="presentation"><form noValidate className="xd-settings-dialog xd-modalities-picker xd-grade-create-dialog" onSubmit={(event) => { void onClass(event).then((saved) => { if (saved) { cancelScheduleEdit(); setCreating(false); } }); }} role="dialog" aria-modal="true" aria-labelledby="xd-new-grade-title">
      ...
      <div className="xd-agenda-two-columns" tabIndex={0} role="region" aria-label="Dados e configurações da grade"><div className="xd-agenda-form">
        <div className="xd-agenda-fields" ref={scheduleFieldsRef}>
          ...
          <label>NOME DA TURMA<input value={classDraft.name} onChange={(event) => setClassDraft({ ...classDraft, name: event.target.value })} required placeholder="EX.: JAZZ INFANTIL" /></label>
          <label>MODALIDADE<select value={classDraft.modalityId} onChange={(event) => { const selected = modalities.find((item) => item.id === event.target.value); setClassDraft({ ...classDraft, modalityId: event.target.value, instructorId: selected?.instructorId || classDraft.instructorId }); }} required><option value="">SELECIONE UMA MODALIDADE</option>{modalities.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label>
          <label>NÍVEL DESTE HORÁRIO<select value={classDraft.level} onChange={(event) => setClassDraft({ ...classDraft, level: event.target.value as ClassLevel })}>{classLevels.map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}</select></label>
          <AgeGroupPicker value={classDraft.ageGroups} onChange={(ageGroups) => setClassDraft({ ...classDraft, ageGroups, ageGroup: ageGroups[0] })} />
          <label>PROFESSOR DESTE HORÁRIO<select value={classDraft.instructorId} onChange={(event) => setClassDraft({ ...classDraft, instructorId: event.target.value })} required><option value="">SELECIONE UM PROFESSOR</option>{instructors.map((instructor) => <option key={instructor.id} value={instructor.id}>{instructor.fullName}</option>)}</select></label>
          <label>SALA<select value={classDraft.roomId} onChange={(event) => { const room = rooms.find((item) => item.id === event.target.value); setClassDraft({ ...classDraft, roomId: event.target.value, capacity: room?.capacity ? String(room.capacity) : classDraft.capacity }); }} required><option value="">SELECIONE UMA SALA</option>{rooms.map((room) => <option key={room.id} value={room.id}>{room.name}{room.capacity ? ` · ${room.capacity} VAGAS` : ""}</option>)}</select></label>
          <label>COR DESTE HORÁRIO<input type="color" value={classDraft.scheduleColor} onChange={(event) => setClassDraft({ ...classDraft, scheduleColor: event.target.value })} /></label>
          <label>VAGAS<input type="number" min="1" value={classDraft.capacity} onChange={(event) => setClassDraft({ ...classDraft, capacity: event.target.value })} /></label>
        </div>
      ...
      return <article key={day} className={schedules.length ? "is-selected" : ""}>
        ...
        {schedules.length ? schedules.map((schedule) => {
          ...
          return <span key={`${schedule.weekday}-${schedule.startsAt}-${schedule.index}`} className={editingIndex === schedule.index ? "is-editing" : ""} style={{ borderColor: schedule.color }} role={onEdit ? "button" : undefined} tabIndex={onEdit ? 0 : undefined} title={onEdit ? "Clique para editar este horário" : undefined} aria-label={onEdit ? `Editar horário de ${weekday[schedule.weekday]} às ${schedule.startsAt}` : undefined} onClick={() => onEdit?.(schedule.index)} onKeyDown={(event) => { if (onEdit && (event.key === "Enter" || event.key === " ")) { event.preventDefault(); onEdit(schedule.index); } }}>
            ...
            <small>{classLevelLabel(schedule.level)} · {ageGroupsLabel(schedule.ageGroups)}</small>
...
function GradeScheduleSummary({ schedules }: { schedules: Group["schedules"] }) {
  ...
  for (const schedule of [...schedules].sort((left, right) => left.weekday - right.weekday || left.startsAt.localeCompare(right.startsAt))) {
    const key = [schedule.instructorId, schedule.level, schedule.startsAt, schedule.endsAt, schedule.roomId, normalizeAgeGroups(schedule.ageGroups).join(","), schedule.color, schedule.capacity ?? "", JSON.stringify(schedule.settings)].join("|");
  ...
  return <section className="xd-grade-schedule-summary" aria-label="Horários cadastrados da turma">
    <div className="xd-grade-schedule-table" role="table">
      <div className="xd-grade-schedule-table-header" role="row"><span role="columnheader">PROFESSOR</span><span role="columnheader">NÍVEL</span><span role="columnheader">HORÁRIO</span><span role="columnheader">DIAS DA SEMANA</span><span role="columnheader">SALA</span><span role="columnheader">PÚBLICO</span></div>
      {rows.map(({ schedule, weekdays }) => <div className="xd-grade-schedule-table-row" role="row" key={`${schedule.id}-${weekdays.join("-")}`} style={{ borderLeftColor: schedule.color }}>
        <strong role="cell">{schedule.instructorName || "A DEFINIR"}</strong>
        <strong role="cell">{classLevelLabel(schedule.level)}</strong>
        ...
        <strong role="cell">{ageGroupsLabel(schedule.ageGroups)}</strong>
      </div>)}
    </div>
  </section>;
}
...
function LegacyGradeSettingsDialog({ group, modalities, instructors, rooms, saving, onClose, onSave }: { group: Group; modalities: Workspace["modalities"]; instructors: Workspace["instructors"]; rooms: Workspace["rooms"]; saving: boolean; onClose: () => void; onSave: (draft: ClassDraft) => Promise<boolean> }) {
  const [draft, setDraft] = useState<ClassDraft>({ name: group.name, modalityId: group.modalityId, level: group.schedules[0]?.level ?? group.level, ageGroup: group.schedules[0]?.ageGroup ?? "ADULTO", ageGroups: group.schedules[0]?.ageGroups ?? [group.schedules[0]?.ageGroup ?? "ADULTO"], roomId: group.schedules[0]?.roomId ?? "", instructorId: group.schedules[0]?.instructorId ?? "", color: "#7435D9", scheduleColor: group.schedules[0]?.color ?? "#7435D9", capacity: group.schedules[0]?.capacity ? String(group.schedules[0].capacity) : "", sourceType: group.sourceType, weekdays: group.schedules.map((schedule) => schedule.weekday), startsAt: group.schedules[0]?.startsAt ?? "19:00", durationMinutes: String(minutesBetween(group.schedules[0]?.startsAt, group.schedules[0]?.endsAt) ?? 60), schedules: group.schedules.map((schedule) => ({ id: schedule.id, weekday: schedule.weekday, startsAt: schedule.startsAt, endsAt: schedule.endsAt, roomId: schedule.roomId, instructorId: schedule.instructorId, level: schedule.level, ageGroup: schedule.ageGroup, ageGroups: schedule.ageGroups, color: schedule.color, capacity: schedule.capacity ? String(schedule.capacity) : "", settings: schedule.settings })), settings: group.schedules[0]?.settings ?? group.settings });
  const [scheduleDraft, setScheduleDraft] = useState({ weekday: 1, startsAt: "19:00", durationMinutes: "60" });
  const modality = modalities.find((item) => item.id === draft.modalityId);
  const scheduleEnd = endTime(scheduleDraft.startsAt, Number(scheduleDraft.durationMinutes));
  function addSchedule() { if (!scheduleEnd || !draft.roomId || !draft.instructorId) return; const next: ScheduleDraft = { weekday: scheduleDraft.weekday, startsAt: scheduleDraft.startsAt, endsAt: scheduleEnd, roomId: draft.roomId, instructorId: draft.instructorId, level: draft.level, ageGroup: draft.ageGroup, ageGroups: draft.ageGroups, color: draft.scheduleColor, capacity: draft.capacity, settings: draft.settings }; if (draft.schedules.some((item) => item.weekday === next.weekday && item.startsAt === next.startsAt && item.endsAt === next.endsAt)) return; setDraft({ ...draft, schedules: [...draft.schedules, next].sort((left, right) => left.weekday - right.weekday || left.startsAt.localeCompare(right.startsAt)) }); }
...
function GradeSettingsDialog({ group, modalities, instructors, rooms, saving, onClose, onSave }: { group: Group; modalities: Workspace["modalities"]; instructors: Workspace["instructors"]; rooms: Workspace["rooms"]; saving: boolean; onClose: () => void; onSave: (draft: ClassDraft) => Promise<boolean> }) {
  const [draft, setDraft] = useState<ClassDraft>({
    ...
    level: group.schedules[0]?.level ?? group.level,
    ageGroup: group.schedules[0]?.ageGroup ?? "ADULTO",
    ageGroups: group.schedules[0]?.ageGroups ?? [group.schedules[0]?.ageGroup ?? "ADULTO"],
    roomId: group.schedules[0]?.roomId ?? "",
    instructorId: group.schedules[0]?.instructorId ?? "",
    color: "#7435D9",
    scheduleColor: group.schedules[0]?.color ?? "#7435D9",
    ...
    schedules: group.schedules.map((schedule) => ({ id: schedule.id, weekday: schedule.weekday, startsAt: schedule.startsAt, endsAt: schedule.endsAt, roomId: schedule.roomId, instructorId: schedule.instructorId, level: schedule.level, ageGroup: schedule.ageGroup, ageGroups: schedule.ageGroups, color: schedule.color, capacity: schedule.capacity ? String(schedule.capacity) : "", settings: schedule.settings })),
  ...
  return <div className="xd-contract-overlay" role="presentation"><form className="xd-settings-dialog xd-grade-create-dialog xd-grade-edit-dialog" onSubmit={(event) => { void submit(event); }} role="dialog" aria-modal="true" aria-labelledby="xd-edit-grade-title">
    ...
    <div className="xd-agenda-two-columns" tabIndex={0} role="region" aria-label="Dados e configurações da grade"><div className="xd-agenda-form">
      <div className="xd-agenda-fields" ref={scheduleFieldsRef}>
        ...
        <label>NÍVEL DESTE HORÁRIO<select value={draft.level} onChange={(event) => setDraft({ ...draft, level: event.target.value as ClassLevel })}>{classLevels.map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}</select></label>
        <AgeGroupPicker value={draft.ageGroups} onChange={(ageGroups) => setDraft({ ...draft, ageGroups, ageGroup: ageGroups[0] })} />
        ...
        <label>COR DESTE HORÁRIO<input type="color" value={draft.scheduleColor} onChange={(event) => setDraft({ ...draft, scheduleColor: event.target.value })} /></label>
...
function Occupation({ groups, rooms, rentals }: { groups: Group[]; rooms: Workspace["rooms"]; rentals: Workspace["rentals"] }) {
  const slots = groups.flatMap((group) => group.schedules.map((schedule) => ({ id: schedule.id, roomName: schedule.roomName || "SALA A DEFINIR", weekday: schedule.weekday, startsAt: schedule.startsAt, endsAt: schedule.endsAt, label: group.name, modality: group.modality, color: schedule.color, students: group.students.length, capacity: group.capacity })));
  const roomNames = Array.from(new Set([...rooms.map((room) => room.name), ...slots.map((slot) => slot.roomName)])).sort(naturalCompare);
  ...
  const boardHeight = Math.max(338, ((endsAt - startsAt) / 60) * pixelsPerHour);
  const visibleDays = [1, 2, 3, 4, 5, 6, 0];
  return <div className="xd-agenda-stack"><section className="xd-occupation"><header><span><Clock3 size={18} /> OCUPAÇÃO DE HORÁRIOS</span><small>{roomNames.length ? `${roomNames.length} SALA(S) EM CADA DIA` : "SEM SALAS CADASTRADAS"}</small></header>{roomNames.length ? <div className="xd-occupation-days">{visibleDays.map((day) => <OccupationWeekday key={day} day={day} roomNames={roomNames} slots={slots.filter((slot) => slot.weekday === day)} startsAt={startsAt} pixelsPerHour={pixelsPerHour} boardHeight={boardHeight} />)}</div> : <p className="xd-agenda-empty">CADASTRE SALAS PARA VISUALIZAR A OCUPAÇÃO DA SEMANA.</p>}</section>{rentals.length ? <section className="xd-occupation-rentals"><header><span>LOCAÇÕES AVULSAS</span><small>EVENTOS POR DATA</small></header>{rentals.map((rental) => <article key={rental.id}><i /><div><strong>{rental.roomName} · {rental.renterName}</strong><small>{dateTime(rental.startsAt)} — {dateTime(rental.endsAt)}</small></div></article>)}</section> : null}</div>;
}
function OccupationWeekday({ day, roomNames, slots, startsAt, pixelsPerHour, boardHeight }: { day: number; roomNames: string[]; slots: OccupationSlot[]; startsAt: number; pixelsPerHour: number; boardHeight: number }) {
  const freeRooms = roomNames.filter((roomName) => !slots.some((slot) => slot.roomName === roomName)).length;
  return <section className="xd-occupation-weekday"><header><div><small>DIA DA SEMANA</small><h2>{weekday[day]}</h2></div><span>{slots.length ? `${slots.length} HORÁRIO(S) · ${freeRooms} SALA(S) LIVRE(S) O DIA TODO` : "TODAS AS SALAS LIVRES"}</span></header><div className="xd-occupation-scroll"><div className="xd-occupation-weekday-board" style={{ "--occupation-height": `${boardHeight}px`, "--occupation-hour": `${pixelsPerHour}px`, "--occupation-rooms": roomNames.length } as CSSProperties}><aside>{Array.from({ length: boardHeight / pixelsPerHour + 1 }, (_, index) => <time key={startsAt + index * 60} style={{ top: `${42 + index * pixelsPerHour}px` }}>{formatHour(startsAt + index * 60)}</time>)}</aside>{roomNames.map((roomName) => <OccupationRoom key={roomName} name={roomName} slots={slots.filter((slot) => slot.roomName === roomName)} startsAt={startsAt} pixelsPerHour={pixelsPerHour} />)}</div></div></section>;
}
function OccupationRoom({ name, slots, startsAt, pixelsPerHour }: { name: string; slots: OccupationSlot[]; startsAt: number; pixelsPerHour: number }) {
  const positioned = positionOccupationSlots(slots);
  return <section className="xd-occupation-room"><header><strong>{name}</strong><small>{slots.length ? `${slots.length} HORÁRIO(S)` : "LIVRE"}</small></header><div>{slots.length ? positioned.map(({ slot, lane, lanes }) => { const start = timeMinutes(slot.startsAt); const end = timeMinutes(slot.endsAt); const width = `calc(${100 / lanes}% - 6px)`; return <article key={slot.id} title={`${slot.label} · ${slot.startsAt}–${slot.endsAt}`} style={{ top: `${((start - startsAt) / 60) * pixelsPerHour}px`, height: `${Math.max(42, ((end - start) / 60) * pixelsPerHour - 4)}px`, left: `calc(${(lane / lanes) * 100}% + 3px)`, width, background: slot.color }}><time>{slot.startsAt} – {slot.endsAt}</time><strong>{slot.label}</strong>{slot.modality ? <span>{slot.modality}</span> : null}<small>{slot.students}{slot.capacity ? ` / ${slot.capacity}` : ""} ALUNOS</small></article>; }) : <p>LIVRE O DIA TODO</p>}</div></section>;
}
...
function calendarRange(month: Date) { const days = calendarCells(month); return { from: isoDay(days[0]), to: isoDay(days[days.length - 1]) }; }
...
function formatDay(value: string) { return new Intl.DateTimeFormat("pt-BR", { weekday: "long", day: "2-digit", month: "2-digit" }).format(new Date(`${value}T12:00:00`)).toUpperCase(); }
function attendanceLabel(value: TrialAppointment["attendanceStatus"]) { return value === "COMPARECEU" ? "COMPARECEU" : value === "FALTOU" ? "FALTOU" : value === "CANCELADO" ? "CANCELADO" : value === "NAO_INFORMADO" ? "NÃO INFORMADO" : "AGENDADO"; }
