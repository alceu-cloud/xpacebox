...
const emptyScheduleDraft = (): ScheduleDraft => ({ weekday: 1, startsAt: "19:00", endsAt: "20:00", roomId: "", instructorId: "", ageGroup: "ADULTO", color: "#7435D9", capacity: "", settings: emptyGradeSettings() });
...
function GradeScheduleSummary({ schedules, level }: { schedules: Group["schedules"]; level: ClassLevel }) {
  ...
  for (const schedule of [...schedules].sort((left, right) => left.weekday - right.weekday || left.startsAt.localeCompare(right.startsAt))) {
    const key = [schedule.instructorId, schedule.startsAt, schedule.endsAt, schedule.roomId, schedule.ageGroup, schedule.color, schedule.capacity ?? "", JSON.stringify(schedule.settings)].join("|");
  ...
  return <section className="xd-grade-schedule-summary" aria-label="Horários cadastrados da turma">
    <div className="xd-grade-schedule-table" role="table">
      ...
      {rows.map(({ schedule, weekdays }) => <div className="xd-grade-schedule-table-row" role="row" key={`${schedule.id}-${weekdays.join("-")}`} style={{ borderLeftColor: schedule.color }}>
        ...
        <strong role="cell">{classLevelLabel(level)}</strong>
