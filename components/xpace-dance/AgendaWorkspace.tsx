...
type ClassDraft = { name: string; modalityId: string; level: ClassLevel; ageGroup: AgeGroup; roomId: string; instructorId: string; color: string; scheduleColor: string; capacity: string; sourceType: "CONTRATO" | "SERVICO"; weekdays: number[]; startsAt: string; durationMinutes: string; schedules: ScheduleDraft[]; settings: GradeSettings };
...
const emptyScheduleDraft = (): ScheduleDraft => ({ weekday: 1, startsAt: "19:00", endsAt: "20:00", roomId: "", instructorId: "", level: "INICIANTE", ageGroup: "ADULTO", color: "#7435D9", capacity: "", settings: emptyGradeSettings() });
const emptyClassDraft = (): ClassDraft => ({ name: "", modalityId: "", level: "INICIANTE", ageGroup: "ADULTO", roomId: "", instructorId: "", color: "#7435D9", scheduleColor: "#7435D9", capacity: "", sourceType: "CONTRATO", weekdays: [], startsAt: "19:00", durationMinutes: "60", schedules: [], settings: emptyGradeSettings() });
...
export default function AgendaWorkspace() {
  ...
  async function submitClass(event: FormEvent): Promise<boolean> {
    ...
    const hasInvalidSchedule = classDraft.schedules.some((schedule) => !schedule.roomId || !schedule.instructorId || !schedule.startsAt || !schedule.endsAt || schedule.endsAt <= schedule.startsAt);
...
function addSchedulesToDraft(draft: ClassDraft, selection: ScheduleSelection, endsAt: string) {
  const scheduleKey = (schedule: Pick<ScheduleDraft, "weekday" | "startsAt" | "endsAt" | "roomId" | "instructorId" | "level" | "ageGroup">) => [schedule.weekday, schedule.startsAt, schedule.endsAt, schedule.roomId, schedule.instructorId, schedule.level, schedule.ageGroup].join(":");
  const existing = new Set(draft.schedules.map(scheduleKey));
  const additions = [...new Set(selection.weekdays)]
    .filter((day) => !existing.has(scheduleKey({ weekday: day, startsAt: selection.startsAt, endsAt, roomId: draft.roomId, instructorId: draft.instructorId, level: draft.level, ageGroup: draft.ageGroup })))
      ...
      return <article key={day} className={schedules.length ? "is-selected" : ""}>
        ...
        {schedules.length ? schedules.map((schedule) => {
          ...
          return <span key={`${schedule.weekday}-${schedule.startsAt}-${schedule.index}`} style={{ borderColor: schedule.color }}>
            ...
            <small>{classLevelLabel(schedule.level)} · {ageGroupLabel(schedule.ageGroup)}</small>
...
function GradeScheduleSummary({ schedules }: { schedules: Group["schedules"] }) {
  ...
  for (const schedule of [...schedules].sort((left, right) => left.weekday - right.weekday || left.startsAt.localeCompare(right.startsAt))) {
    const key = [schedule.instructorId, schedule.level, schedule.startsAt, schedule.endsAt, schedule.roomId, schedule.ageGroup, schedule.color, schedule.capacity ?? "", JSON.stringify(schedule.settings)].join("|");
...
function Occupation({ groups, rooms, rentals }: { groups: Group[]; rooms: Workspace["rooms"]; rentals: Workspace["rentals"] }) {
  ...
  const roomNames = Array.from(new Set([...rooms.map((room) => room.name), ...slots.map((slot) => slot.roomName)])).sort((left, right) => left.localeCompare(right));
