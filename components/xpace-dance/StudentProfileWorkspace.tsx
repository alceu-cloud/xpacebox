...
export default function StudentProfileWorkspace({ studentId, onBack }: { studentId: string; onBack: () => void }) {
  ...
  return <section className="xd-student-profile">
    ...
    {benefitOpen ? <BenefitDialog profiles={profile.benefitProfiles} current={activeBenefit} selected={benefitProfileId} note={benefitNote} saving={saving} onSelect={setBenefitProfileId} onNote={setBenefitNote} onClose={() => setBenefitOpen(false)} onSubmit={saveBenefit} onEnd={endBenefit} /> : null}
  </section>;
}
...
function BenefitDialog({ profiles, current, selected, note, saving, onSelect, onNote, onClose, onSubmit, onEnd }: { profiles: Profile["benefitProfiles"]; current: Profile["benefits"][number] | undefined; selected: string; note: string; saving: boolean; onSelect: (value: string) => void; onNote: (value: string) => void; onClose: () => void; onSubmit: (event: FormEvent) => void; onEnd: () => void }) { const activeProfiles = profiles.filter((profile) => profile.active); const selectedId = selected || current?.profileId || activeProfiles[0]?.id || ""; return <div className="xd-profile-overlay" role="presentation"><form className="xd-profile-dialog" onSubmit={(event) => { if (!selected) onSelect(selectedId); onSubmit(event); }}><header><span>BENEFÍCIO DO ALUNO</span><button type="button" title="Fechar" onClick={onClose}>×</button></header><h2>VIP E BOLSAS.</h2><p>Um benefício ativo por aluno. A condição será copiada para novos contratos, sem alterar os que já existem.</p><label>PERFIL<select value={selectedId} onChange={(event) => onSelect(event.target.value)} required><option value="">SELECIONE</option>{activeProfiles.map((profile) => <option value={profile.id} key={profile.id}>{profile.name} · {benefitLabel(profile.discountType, profile.discountValue)}</option>)}</select></label><label>OBSERVAÇÃO<input value={note} onChange={(event) => onNote(event.target.value)} placeholder="EX.: BOLSA SOCIAL APROVADA" /></label><footer>{current ? <button className="xd-danger-link" type="button" disabled={saving} onClick={onEnd}>ENCERRAR ATUAL</button> : <span />}{activeProfiles.length ? <button type="submit" className="xd-primary" disabled={saving}>APLICAR BENEFÍCIO</button> : <small>CADASTRE PERFIS VIP OU BOLSA NO CATÁLOGO DE BENEFÍCIOS.</small>}</footer></form></div>; }
