...
function saleStateLabel(sale: Profile["sales"][number]) {
  if (!sale.signatureRequired || sale.signatureStatus === "ASSINADA" || sale.status === "CANCELADA") return labelSaleStatus(sale.status);
  return `${labelSaleStatus(sale.status)} · ASSINATURA ${labelSignature(sale.signatureStatus)}`;
}
