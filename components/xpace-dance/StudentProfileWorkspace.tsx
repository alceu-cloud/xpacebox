...
type SalePlan = { id: string; name: string; description: string; billingInterval: string; durationMonths: number; amountCents: number; renewsAutomatically: boolean; active: boolean; modalityRules: Array<{ modalityId?: string }>; catalogSettings: { sendForSignature?: boolean; enrollmentFeeEnabled?: boolean; enrollmentServiceId?: string } };
...
export default function StudentProfileWorkspace({ studentId }: { studentId: string }) {
  ...
  async function createSale(input: { planId: string; classGroupId: string; saleOn: string; amountCents: number }) {
    setSaving(true); setNotice("");
    try {
      const payload = await request<{ pendingSignature?: boolean }>("/api/xpace/contratos", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: "CREATE_CONTRACT", contract: { studentId, planId: input.planId, classGroupId: input.classGroupId, saleOn: input.saleOn, amountCents: input.amountCents } }) });
  ...
  async function sendForSignature(saleId: string) {
    if (!window.confirm("ENVIAR ESTE CONTRATO PARA ASSINATURA? CONFIRA ANTES O MODELO PDF E OS DADOS DO ALUNO.")) return;
    setSaving(true); setNotice("");
    try {
      const payload = await request<{ signatureUrl?: string; sandbox?: boolean }>("/api/xpace/assinaturas", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ saleId }) });
      await loadProfile();
      setNotice(payload.sandbox ? "SOLICITAÇÃO ENVIADA À AUTENTIQUE EM MODO DE TESTE. O CONTRATO SÓ ATIVA QUANDO O WEBHOOK CONFIRMAR A ASSINATURA." : "SOLICITAÇÃO ENVIADA À AUTENTIQUE. O CONTRATO SÓ ATIVA QUANDO A ASSINATURA FOR CONFIRMADA.");
      if (payload.signatureUrl) window.open(payload.signatureUrl, "_blank", "noopener,noreferrer");
    } catch (error) { setNotice(error instanceof Error ? error.message : "NÃO FOI POSSÍVEL ENVIAR O CONTRATO PARA ASSINATURA."); }
    finally { setSaving(false); }
  }

  async function updateContractStatus(contractId: string, status: "PAUSADO" | "ENCERRADO" | "CANCELADO") {
    const confirmation = status === "CANCELADO" ? "Cancelar este contrato e manter o histórico financeiro?" : status === "ENCERRADO" ? "Encerrar este contrato e cancelar somente as cobranças futuras em aberto?" : "Suspender este contrato e cancelar as cobranças futuras em aberto a partir de hoje? O histórico não será apagado.";
    if (!window.confirm(confirmation)) return;
    const statusNote = status === "CANCELADO" ? window.prompt("MOTIVO DO CANCELAMENTO:") ?? "" : "";
    if (status === "CANCELADO" && !statusNote) return;
    setSaving(true); setNotice("");
    try {
      await request("/api/xpace/contratos", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: "SET_CONTRACT_STATUS", contract: { id: contractId, status, statusNote } }) });
      await loadProfile(); setNotice(status === "PAUSADO" ? "CONTRATO SUSPENSO. AS COBRANÇAS FUTURAS EM ABERTO FORAM CANCELADAS E O HISTÓRICO FOI PRESERVADO." : "CONTRATO ATUALIZADO E HISTÓRICO PRESERVADO.");
  ...
  return <section className="xd-student-profile">
    ...
    {tab === "CONTRATOS" ? <Contracts contracts={profile.contracts} saving={saving} onRenew={(contract) => void openSale({ planId: contract.planId, startsOn: addDays(contract.endsOn, 1) })} onStatus={updateContractStatus} /> : null}
...
function Contracts({ contracts, saving, onRenew, onStatus }: { contracts: Profile["contracts"]; saving: boolean; onRenew: (contract: Profile["contracts"][number]) => void; onStatus: (contractId: string, status: "PAUSADO" | "ENCERRADO" | "CANCELADO") => void }) {
...
function SaleDialog({ plans, classGroups, modalities, services, preset, saving, onClose, onSubmit }: { plans: SalePlan[]; classGroups: SaleClassGroup[]; modalities: SaleModality[]; services: SaleService[]; preset: { planId?: string; startsOn?: string }; saving: boolean; onClose: () => void; onSubmit: (input: { planId: string; classGroupId: string; saleOn: string; amountCents: number }) => Promise<void> }) {
  ...
  const enrollment = plan?.catalogSettings.enrollmentFeeEnabled ? services.find((service) => service.id === plan.catalogSettings.enrollmentServiceId) : undefined;

  async function submit(event: FormEvent) {
    event.preventDefault();
    if (!plan || !selectedGroup) return;
    await onSubmit({ planId: plan.id, classGroupId: selectedGroup.id, saleOn, amountCents: finalValue });
  }

  function choosePlan(id: string) { setPlanId(id); setClassGroupId(""); setDiscount(""); setPickerOpen(false); }

  return <div className="xd-profile-overlay" role="presentation"><form className="xd-profile-dialog xd-sale-dialog xd-sale-primary-dialog" onSubmit={submit}>
    <header><span><FileSignature size={17} /> NOVA VENDA</span><button type="button" title="Fechar" onClick={onClose}>×</button></header>
    <h2>REGISTRAR VENDA.</h2>
    <p>A data da venda define o início do contrato e o dia de vencimento de todas as cobranças recorrentes.</p>
    <div className="xd-sale-start"><label>DATA DA VENDA<input type="date" value={saleOn} onChange={(event) => setSaleOn(event.target.value)} required /></label><button type="button" className="xd-primary" onClick={() => setPickerOpen(true)}><Plus size={16} /> {plan ? "ALTERAR CONTRATO" : "ADICIONAR CONTRATO"}</button></div>
    {plan ? <div className="xd-sale-plan-preview"><div><strong>{plan.name}</strong><span>{cycleLabel(plan.durationMonths)} · {plan.renewsAutomatically ? "RENOVAÇÃO AUTOMÁTICA" : "RENOVAÇÃO MANUAL"}</span></div><b>{currency(plan.amountCents)}{plan.billingInterval === "MENSAL" ? " / MÊS" : ""}</b><small>{enrollment ? `TAXA DE ADESÃO: ${currency(enrollment.salePriceCents)}` : "SEM TAXA DE ADESÃO"}</small>{plan.catalogSettings?.sendForSignature ? <small>VAI AGUARDAR A ASSINATURA PARA ATIVAR O CONTRATO.</small> : null}</div> : <button type="button" className="xd-sale-empty-plan" onClick={() => setPickerOpen(true)}><Plus size={18} /><span>ADICIONE O CONTRATO DESTA VENDA</span><small>BUSQUE POR NOME, MODALIDADE OU PERÍODO.</small></button>}
    <label>GRADE DE AULA<select value={classGroupId} onChange={(event) => setClassGroupId(event.target.value)} required disabled={!plan}><option value="">{plan ? "SELECIONE A GRADE" : "ADICIONE O CONTRATO PRIMEIRO"}</option>{availableGroups.map((group) => <option key={group.id} value={group.id}>{group.name} · {scheduleLabel(group.schedules)}</option>)}</select>{plan && !availableGroups.length ? <small>CADASTRE UMA GRADE ATIVA PARA UMA DAS MODALIDADES DESTE CONTRATO ANTES DE VENDER.</small> : null}</label>
    <label>DESCONTO (R$)<input inputMode="numeric" value={discount} onChange={(event) => setDiscount(currencyInput(event.target.value))} placeholder="R$ 0,00" disabled={!plan} /></label>
    <div className="xd-sale-total"><span>VALOR DO CONTRATO</span><strong>{currency(finalValue)}</strong></div>
    ...
    {pickerOpen ? <PlanPicker plans={plans} modalities={modalities} services={services} selectedPlanId={planId} onClose={() => setPickerOpen(false)} onChoose={choosePlan} /> : null}
  </form></div>;
}
