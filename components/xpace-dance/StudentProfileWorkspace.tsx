...
export default function StudentProfileWorkspace({ studentId }: { studentId: string }) {
  ...
  const syncedSignatureSales = useRef(new Set<string>());
  ...
  useEffect(() => {
    if (tab !== "VENDAS" || !profile) return;
    const saleIds = profile.sales.filter((sale) => sale.signatureStatus === "ENVIADA" && !syncedSignatureSales.current.has(sale.id)).map((sale) => sale.id);
    if (!saleIds.length) return;
    saleIds.forEach((saleId) => syncedSignatureSales.current.add(saleId));
    void (async () => {
      try {
        await request("/api/xpace/assinaturas/status", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ saleIds }) });
        await loadProfile({ silent: true });
      } catch { /* The webhook remains the primary source; this is only a one-time fallback per page load. */ }
    })();
  }, [tab, profile]);

  const pendingCancellationKey = [...new Set(profile?.charges.filter((charge) => ["PENDING", "PROCESSING"].includes(charge.cancellationStatus)).map((charge) => charge.contractId) ?? [])].sort().join(",");
  useEffect(() => {
    ...
    const retry = async () => {
      ...
      running = true;
      try {
        for (const contractId of pendingCancellationKey.split(",")) {
          if (stopped) break;
          await request("/api/xpace/cobrancas/cancelamentos", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ contractId }) });
        }
        if (!stopped) await loadProfile({ silent: true });
      } catch { /* Persistent cancellation state and error remain visible. */ }
      finally { running = false; }
    };
    void retry();
    const timer = window.setInterval(() => void retry(), 30_000);
    return () => { stopped = true; window.clearInterval(timer); };
  }, [studentId, pendingCancellationKey]);
  ...
  async function createSale(input: { planId: string; classGroupIds: string[]; saleOn: string; firstDueOn: string; discountType: "PERCENTUAL" | "FIXO"; discountValue: number; enrollmentFeeEnabled: boolean; paymentMethod: "PIX" | "CARTAO" }) {
    ...
    try {
      ...
      if (payload.pendingSignature && payload.saleId) {
        try {
          const signature = await request<{ sandbox?: boolean }>("/api/xpace/assinaturas", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ saleId: payload.saleId }) });
          setNotice(signature.sandbox ? "VENDA REGISTRADA. CONTRATO ENVIADO AUTOMATICAMENTE PARA ASSINATURA EM MODO DE TESTE." : "VENDA REGISTRADA. CONTRATO ENVIADO AUTOMATICAMENTE PARA ASSINATURA.");
        } catch (error) {
          setNotice(`VENDA REGISTRADA, MAS O ENVIO AUTOMÁTICO DA ASSINATURA FALHOU: ${error instanceof Error ? error.message : "TENTE REENVIAR."}`);
        }
      } else {
        setNotice("VENDA REGISTRADA. O CONTRATO E AS COBRANÇAS MENSAIS FORAM CRIADOS.");
      }
      await loadProfile(); setTab("VENDAS");
    } catch (error) {
  ...
  async function sendForSignature(saleId: string, resend = false) {
    if (!window.confirm(resend ? "REENVIAR ESTE CONTRATO PARA ASSINATURA POR E-MAIL?" : "ENVIAR ESTE CONTRATO PARA ASSINATURA POR E-MAIL?")) return;
    setSaving(true); setNotice("");
    try {
  ...
  async function updateContractStatus(contractId: string, status: "PAUSADO" | "ENCERRADO" | "CANCELADO", effectiveOn = "") {
    ...
    const confirmation = isScheduled ? `Agendar o encerramento para ${date(effectiveOn)}? As cobranças com vencimento nessa data ou depois serão canceladas.` : status === "CANCELADO" ? "Cancelar este contrato e manter o histórico financeiro?" : status === "ENCERRADO" ? "Encerrar este contrato? Sem recebimento, a venda será cancelada sem efetivação e todas as cobranças abertas serão canceladas. Se já houve recebimento, o histórico será mantido e apenas cobranças futuras abertas serão canceladas." : "Suspender este contrato e cancelar as cobranças futuras em aberto a partir de hoje? O histórico não será apagado.";
  ...
  return <section className="xd-student-profile">
    ...
    {tab === "VENDAS" ? <Sales sales={profile.sales} saving={saving} onSell={() => void openSale()} onSend={(saleId, resend) => void sendForSignature(saleId, resend)} /> : null}
...
function Sales({ sales, saving, onSell, onSend }: { sales: Profile["sales"]; saving: boolean; onSell: () => void; onSend: (saleId: string, resend: boolean) => void }) {
  ...
  return <div className="xd-profile-content"><section className="xd-profile-panel xd-sales-history"><header><div><span><CircleDollarSign size={18} /> HISTÓRICO DE VENDAS</span><small>VENDA, ASSINATURA E CANCELAMENTO FICAM RASTREÁVEIS.</small></div><button type="button" className="xd-sell-contract" onClick={onSell}><FileSignature size={18} /><span>NOVA VENDA</span><small>VENDER CONTRATO</small></button></header><div className="xd-history-filters"><label><Filter size={14} /><select value={period} onChange={(event) => setPeriod(event.target.value)}><option value="TODOS">TODO O PERÍODO</option><option value="MES">MÊS ATUAL</option><option value="TRIMESTRE">TRIMESTRE ATUAL</option><option value="ANO">ANO ATUAL</option></select></label><label><select value={status} onChange={(event) => setStatus(event.target.value)}><option value="TODOS">TODAS AS SITUAÇÕES</option><option value="CONCLUIDA">EFETIVADA</option><option value="EM_PREPARACAO">AGUARDANDO PAGAMENTO</option><option value="PROCESSANDO">PROCESSANDO</option><option value="ERRO">ERRO</option><option value="CANCELADA">CANCELADA SEM EFETIVAÇÃO</option></select></label></div>{visible.length ? <div className="xd-sales-table"><div className="xd-sales-head"><span>CONTRATO</span><span>DATA DA VENDA</span><span>DESCONTO</span><span>VALOR</span><span>SITUAÇÃO</span></div>{visible.map((sale) => <article key={sale.id}><div><strong>#{String(sale.contractNumber).padStart(5, "0")} · {sale.planName}</strong><small>{["CANCELADO", "ENCERRADO"].includes(sale.contractStatus) ? (sale.signatureStatus === "ASSINADA" ? "ASSINATURA REGISTRADA · CONTRATO ENCERRADO" : "CONTRATO ENCERRADO · SEM PENDÊNCIA DE ASSINATURA") : sale.signatureRequired ? `ASSINATURA · ${labelSignature(sale.signatureStatus)}${sale.signatureDueOn && sale.signatureStatus !== "ASSINADA" ? ` · ATÉ ${date(sale.signatureDueOn)}` : ""}` : "SEM ASSINATURA EXTERNA"}</small>{!["CANCELADO", "ENCERRADO"].includes(sale.contractStatus) && sale.signatureRequired && !["ASSINADA", "ENVIADA"].includes(sale.signatureStatus) ? <button type="button" className="xd-secondary" disabled={saving} onClick={() => onSend(sale.id, true)}><Send size={14} /> REENVIAR POR E-MAIL</button> : null}{!["CANCELADO", "ENCERRADO"].includes(sale.contractStatus) && sale.signatureUrl ? <a className="xd-secondary" href={sale.signatureUrl} target="_blank" rel="noreferrer">ABRIR ASSINATURA</a> : null}</div><span>{date(sale.soldAt)}</span><span>{sale.discountCents ? currency(sale.discountCents) : "—"}</span><b>{currency(sale.amountCents)}</b><span className={`xd-sale-state xd-sale-state--${sale.status.toLowerCase()}`}>{saleStateLabel(sale)}{sale.effectiveAt && ["CANCELADO", "ENCERRADO"].includes(sale.contractStatus) ? " · CONTRATO ENCERRADO" : ""}</span></article>)}</div> : <Empty title="SEM VENDAS NESTE FILTRO" copy="A emissão de um contrato aparecerá aqui como venda, sem misturar com recebimentos." />}</section></div>;
}
function Contracts({ contracts, saving, onRenew, onStatus, onDetails }: { contracts: Profile["contracts"]; saving: boolean; onRenew: (contract: Profile["contracts"][number]) => void; onStatus: (contractId: string, status: "PAUSADO" | "ENCERRADO" | "CANCELADO") => void; onDetails: (contract: Profile["contracts"][number]) => void }) {
  ...
  const visible = contracts.filter((contract) => filter === "TODOS" || contract.status === filter).sort((left, right) => contractRank(left.status) - contractRank(right.status) || right.startsOn.localeCompare(left.startsOn));
  function printContract(contract: Profile["contracts"][number]) {
    if (!contract.sale?.signedDocumentUrl) {
      window.alert("O PDF ASSINADO AINDA NÃO ESTÁ DISPONÍVEL PARA IMPRESSÃO.");
      return;
    }
    window.open(contract.sale.signedDocumentUrl, "_blank", "noopener,noreferrer");
  }
  return <div className="xd-profile-content"><section className="xd-profile-panel xd-contract-history"><header><div><span><FileText size={18} /> HISTÓRICO DE CONTRATOS</span><small>ATIVOS APARECEM PRIMEIRO. NENHUM ENCERRAMENTO APAGA O PASSADO.</small></div><label className="xd-history-select"><Filter size={14} /><select value={filter} onChange={(event) => setFilter(event.target.value)}><option value="TODOS">TODOS</option><option value="ATIVO">ATIVOS</option><option value="AGENDADO">AGENDADOS</option><option value="PAUSADO">SUSPENSOS</option><option value="ENCERRADO">ENCERRADOS</option><option value="CANCELADO">CANCELADOS</option></select></label></header>{visible.length ? visible.map((contract) => {
    const canSuspend = ["ATIVO", "AGENDADO"].includes(contract.status);
    const canEnd = ["ATIVO", "PAUSADO", "AGENDADO"].includes(contract.status);
    return <article key={contract.id}><div><strong>#{String(contract.contractNumber).padStart(5, "0")} · {contract.planName}</strong><small>{cycleLabel(contract.durationMonths, contract.billingInterval)} · VÁLIDO DE {date(contract.startsOn)} ATÉ {date(contract.endsOn)}{contract.renewsAutomatically ? " · RENOVAÇÃO AUTOMÁTICA" : ""}</small></div><b>{currency(contract.amountCents)}<small>{billingUnit(contract.billingInterval)}</small></b><span className={`xd-contract-state xd-contract-state--${contract.status.toLowerCase()}`}>{labelContractStatus(contract.status)}</span><div className="xd-contract-actions">{!contract.renewsAutomatically && ["ATIVO", "AGENDADO", "ENCERRADO"].includes(contract.status) ? <button type="button" className="xd-secondary" disabled={saving} onClick={() => onRenew(contract)}><RotateCw size={14} /> RENOVAR</button> : null}<button type="button" className="xd-secondary" onClick={() => onDetails(contract)}>DETALHES</button>{canSuspend ? <button type="button" className="xd-contract-action xd-contract-action--suspend" disabled={saving} onClick={() => onStatus(contract.id, "PAUSADO")}><XCircle size={14} /> SUSPENDER</button> : null}<button type="button" className="xd-contract-action xd-contract-action--print" onClick={() => printContract(contract)}><Printer size={14} /> IMPRIMIR CONTRATO</button>{canEnd ? <button type="button" className="xd-contract-action xd-contract-action--end" disabled={saving} onClick={() => onStatus(contract.id, "ENCERRADO")}><XCircle size={14} /> ENCERRAR</button> : null}</div></article>;
  }) : <Empty title="SEM CONTRATOS NESTE FILTRO" copy="Os contratos emitidos para o aluno ficam aqui, inclusive encerrados e cancelados." />}</section></div>;
}
    ...
    <section className="xd-profile-panel xd-finance-history">
      ...
      {charges.length ? <div className="xd-finance-table xd-finance-table--full">
        ...
        {charges.map((charge) => <article key={charge.id}>
          ...
          <span className={`xd-charge-state xd-charge-state--${charge.status.toLowerCase()}`}>{charge.status}{["PENDING", "PROCESSING"].includes(charge.cancellationStatus) ? <small>CONFIRMANDO CANCELAMENTO NO ASAAS</small> : charge.cancellationStatus === "REVIEW" ? <small>REVISÃO NECESSÁRIA</small> : null}</span>
          <div className="xd-finance-actions">
            ...
            {charge.providerError || charge.cancellationError ? <small className="xd-finance-error">{charge.cancellationError || charge.providerError}</small> : null}
...
function cycleLabel(months: number, interval = "") { if (interval === "DIARIO") return "DIÁRIO"; return months === 1 ? "MENSAL" : months === 6 ? "SEMESTRAL" : months === 12 ? "ANUAL" : `${months} MESES`; }
function billingUnit(interval: string) { return interval === "DIARIO" ? "POR DIA" : "POR MÊS"; }
...
function labelSaleStatus(value: string) { return ({ EM_PREPARACAO: "AGUARDANDO PAGAMENTO", PENDENTE_ASSINATURA: "ASSINATURA PENDENTE", ENVIADA_PARA_ASSINATURA: "ASSINATURA ENVIADA", CONCLUIDA: "EFETIVADA", CANCELADA: "CANCELADA SEM EFETIVAÇÃO", PROCESSANDO: "PROCESSANDO", ERRO: "ERRO" } as Record<string, string>)[value] ?? value; }
function saleStateLabel(sale: Profile["sales"][number]) {
  if (!sale.signatureRequired || sale.status === "CANCELADA") return labelSaleStatus(sale.status);
  return `${labelSaleStatus(sale.status)} · ${sale.signatureStatus === "ASSINADA" ? "ASSINATURA OK" : `ASSINATURA ${labelSignature(sale.signatureStatus)}`}`;
}
