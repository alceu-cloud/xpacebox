...
import { AlertTriangle, CalendarClock, Camera, CheckCircle2, CircleDollarSign, Crown, FileSignature, FileText, Filter, Gift, History, Mail, MessageCircle, MoreHorizontal, Pencil, Plus, Printer, RotateCw, Send, Settings2, StickyNote, WalletCards, XCircle } from "lucide-react";
...
export default function StudentProfileWorkspace({ studentId }: { studentId: string }) {
  ...
  async function createSale(input: { planId: string; startsOn: string; amountCents: number }) {
    ...
    try {
      ...
      setSaleOpen(false); await loadProfile(); setTab("VENDAS"); setNotice(payload.pendingSignature ? "VENDA REGISTRADA E AGUARDANDO A ASSINATURA. AS COBRANÇAS SÓ SERÃO GERADAS APÓS A CONFIRMAÇÃO." : "VENDA REGISTRADA. O CONTRATO E AS COBRANÇAS MENSAIS FORAM CRIADOS.");
  ...
  return <section className="xd-student-profile">
    ...
    {tab === "VENDAS" ? <Sales sales={profile.sales} onSell={() => void openSale()} /> : null}
...
function Sales({ sales, onSell }: { sales: Profile["sales"]; onSell: () => void }) {
  ...
  return <div className="xd-profile-content"><section className="xd-profile-panel xd-sales-history"><header><div><span><CircleDollarSign size={18} /> HISTÓRICO DE VENDAS</span><small>VENDA, ASSINATURA E CANCELAMENTO FICAM RASTREÁVEIS.</small></div><button type="button" className="xd-primary" onClick={onSell}><Plus size={16} /> VENDER CONTRATO</button></header><div className="xd-history-filters"><label><Filter size={14} /><select value={period} onChange={(event) => setPeriod(event.target.value)}><option value="TODOS">TODO O PERÍODO</option><option value="MES">MÊS ATUAL</option><option value="TRIMESTRE">TRIMESTRE ATUAL</option><option value="ANO">ANO ATUAL</option></select></label><label><select value={status} onChange={(event) => setStatus(event.target.value)}><option value="TODOS">TODAS AS SITUAÇÕES</option><option value="CONCLUIDA">CONCLUÍDA</option><option value="PENDENTE_ASSINATURA">ASSINATURA PENDENTE</option><option value="ENVIADA_PARA_ASSINATURA">ENVIADA PARA ASSINATURA</option><option value="PROCESSANDO">PROCESSANDO</option><option value="ERRO">ERRO</option><option value="CANCELADA">CANCELADA</option></select></label></div>{visible.length ? <div className="xd-sales-table"><div className="xd-sales-head"><span>CONTRATO</span><span>DATA DA VENDA</span><span>DESCONTO</span><span>VALOR</span><span>SITUAÇÃO</span></div>{visible.map((sale) => <article key={sale.id}><div><strong>#{String(sale.contractNumber).padStart(5, "0")} · {sale.planName}</strong><small>{sale.signatureRequired ? `ASSINATURA · ${labelSignature(sale.signatureStatus)}` : "SEM ASSINATURA EXTERNA"}</small></div><span>{dateTime(sale.soldAt)}</span><span>{sale.discountCents ? currency(sale.discountCents) : "—"}</span><b>{currency(sale.amountCents)}</b><span className={`xd-sale-state xd-sale-state--${sale.status.toLowerCase()}`}>{labelSaleStatus(sale.status)}</span></article>)}</div> : <Empty title="SEM VENDAS NESTE FILTRO" copy="A emissão de um contrato aparecerá aqui como venda, sem misturar com recebimentos." />}</section></div>;
}
...
function inPeriod(value: string, period: string) { const dateValue = new Date(value); const now = new Date(); if (period === "TODOS") return true; if (period === "MES") return dateValue.getFullYear() === now.getFullYear() && dateValue.getMonth() === now.getMonth(); if (period === "TRIMESTRE") return dateValue.getFullYear() === now.getFullYear() && Math.floor(dateValue.getMonth() / 3) === Math.floor(now.getMonth() / 3); return dateValue.getFullYear() === now.getFullYear(); }
