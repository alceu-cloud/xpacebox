...
export default function DanceWorkspace() {
  ...
  const [screen, setScreen] = useState<"HOME" | "COMMUNITY" | "PROFILE" | "AGENDA" | "FINANCE" | "ADMINISTRATIVE" | "CONTRACTS" | "MODALITIES">("HOME");
  const [profileId, setProfileId] = useState("");
  const activeModule = screen === "COMMUNITY" || screen === "PROFILE" ? "CLIENTES" : screen === "AGENDA" ? "AGENDA" : screen === "FINANCE" ? "FINANCEIRO" : screen === "ADMINISTRATIVE" || screen === "CONTRACTS" || screen === "MODALITIES" ? "ADMINISTRATIVO" : null;
  ...
  const topbar = <header className="xd-topbar">
    ...
    {activeModule ? <div className="xd-topbar-context"><button type="button" className="xd-active-module xd-active-module--return" onClick={() => setScreen(screen === "PROFILE" ? "COMMUNITY" : screen === "CONTRACTS" || screen === "MODALITIES" ? "ADMINISTRATIVE" : "HOME")} title={screen === "PROFILE" ? "Voltar à Comunidade" : screen === "CONTRACTS" || screen === "MODALITIES" ? "Voltar ao Administrativo" : "Voltar ao Painel"}><span>MÓDULO ATIVO</span><strong>{activeModule}</strong></button></div> : <button type="button" className="xd-back" onClick={() => router.push("/")}>CENTRAL</button>}
  </header>;
  ...
  if (screen === "ADMINISTRATIVE") return <main className="xd-shell">{topbar}<AdministrativeWorkspace onOpenContracts={() => setScreen("CONTRACTS")} onOpenModalities={() => setScreen("MODALITIES")} /></main>;
