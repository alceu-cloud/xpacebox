  ...
  function renderFields(
    ...
    return (
      <div style={productFieldsStyle}>
        ...
        <label style={productLabelStyle}>MATERIAL<SearchableSelect value={item.materialId ?? ""} onChange={(value) => { const material = materials.find((candidate) => candidate.id === value); if (material?.supplier) setSupplierSelection((current) => ({ ...current, [item.id]: material.supplier })); update("materialId", value); update("engineeringId", ""); }} options={filteredMaterials.map((material) => ({ value: material.id, label: [material.name?.trim() || material.code, material.grammage, material.pressure].map((value) => (value || "").trim()).filter((value) => value && value !== "-").join(" | ") }))} placeholder={selectedSupplier ? "SELECIONE O MATERIAL" : "SELECIONE O FORNECEDOR PRIMEIRO"} inputStyle={productInputStyle} ariaLabel="MATERIAL" disabled={!selectedSupplier} /></label>
