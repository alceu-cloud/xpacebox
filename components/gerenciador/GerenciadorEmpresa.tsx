...
export function ProductCatalogPanel({
  ...
  function PriceSnapshotDetails({ snapshot }: { snapshot?: ProductPriceSnapshot }) {
    ...
    const details = [
      ...
      ...(typeof snapshot.mainAreaM2 === "number" ? [["AREA CAIXA PRINCIPAL", formatNumberValue(snapshot.mainAreaM2, " M2")]] : []),
      ["AREA TOTAL", formatNumberValue(snapshot.totalAreaM2 ?? snapshot.areaM2, " M2")],
  ...
  function renderFields(
    ...
    return (
      <div style={productFieldsStyle}>
        ...
        <label style={productLabelStyle}>MATERIAL<SearchableSelect value={item.materialId ?? ""} onChange={(value) => { const material = materials.find((candidate) => candidate.id === value); if (material?.supplier) setSupplierSelection((current) => ({ ...current, [item.id]: material.supplier })); update("materialId", value); update("engineeringId", ""); }} options={filteredMaterials.map((material) => ({ value: material.id, label: [material.name?.trim() || material.code, material.grammage, material.pressure].map((value) => (value || "").trim()).filter((value) => value && value !== "-").join(" | "), tone: material.specialCondition ? "special" : undefined }))} placeholder={selectedSupplier ? "SELECIONE O MATERIAL" : "SELECIONE O FORNECEDOR PRIMEIRO"} inputStyle={productInputStyle} ariaLabel="MATERIAL" disabled={!selectedSupplier} forceSearchable /></label>
    ...
    <>
      {draft ? (
        ...
        <Panel title="FICHAS TECNICAS DE PRODUTOS" description="CADASTRE A CAIXA PRINCIPAL E OS ACESSORIOS VINCULADOS A CADA FT." actionLabel="+ NOVA FICHA TECNICA" onAction={startCreate}>
          ...
          {fichas.length === 0 ? <div style={emptyListStyle}>NENHUMA FICHA TECNICA CADASTRADA.</div> : filteredFichas.length === 0 ? <div style={emptyListStyle}>NENHUMA FICHA ENCONTRADA PARA OS FILTROS INFORMADOS.</div> : <div style={fichaListStyle}>{filteredFichas.map((ficha) => {
            const client = clientsById.get(ficha.clientId);
            ...
            return <article key={ficha.id} style={fichaRowStyle}>
              <div style={fichaColumnStyle}><small style={fichaColumnLabelStyle}>FT</small><strong style={fichaNumberStyle}>{ficha.ftNumber}</strong></div>
              <div style={fichaColumnStyle}><small style={fichaColumnLabelStyle}>DESCRICAO</small><strong style={fichaReferenceStyle}>{ficha.reference}</strong></div>
              <div style={fichaColumnStyle}><small style={fichaColumnLabelStyle}>CLIENTE</small><strong style={fichaClientStyle}>{clientName}</strong></div>
              <div style={fichaColumnStyle}><small style={fichaColumnLabelStyle}>{engineeringPrice > 0 ? "PRECO ENGENHARIA" : "PRECO CADASTRADO"}</small><strong style={fichaMetricPriceStyle}>{displayedPrice > 0 ? formatCurrencyValue(displayedPrice) : "SEM PRECO"}</strong></div>
              <div style={fichaColumnStyle}><small style={fichaColumnLabelStyle}>AREA TOTAL</small><strong style={fichaMetricValueStyle}>{totalAreaM2 > 0 ? `${totalAreaM2.toLocaleString("pt-BR", { minimumFractionDigits: 4, maximumFractionDigits: 4 })} M2` : "NAO CALCULADA"}</strong></div>
              <div style={fichaColumnStyle}><small style={fichaColumnLabelStyle}>EMPRESA / ACESSORIOS</small><strong style={fichaMetaStyle}>{ficha.company} · {ficha.accessories.length} ACESSORIO(S)</strong></div>
...
const panelTitleStyle = { margin: 0, color: "#e6007e", fontWeight: 900, ...ui.title };
const panelTextStyle = { margin: "14px 0 0", color: "#667085", fontSize: 16, fontWeight: 800 };
const orangeButtonStyle = { border: "none", background: "linear-gradient(180deg,#ff5a00,#df7b00)", color: "#fff", cursor: "pointer", ...ui.button };
const tableWrapStyle = { overflowX: "auto" as const, border: "1px solid var(--xb-line)", borderRadius: "var(--xb-radius-card)", background: "#fff" };
const tableStyle = { width: "100%", minWidth: 680, tableLayout: "auto" as const, borderCollapse: "collapse" as const };
const thStyle = { padding: "18px 14px", textAlign: "center" as const, background: "rgba(255,0,135,.08)", color: "#d60072", fontSize: 16, fontWeight: 900, letterSpacing: 0, borderBottom: "1px solid rgba(255,0,135,.22)", whiteSpace: "normal" as const, overflowWrap: "anywhere" as const };
const strongCellStyle = { padding: "18px 14px", borderBottom: "1px solid rgba(255,0,135,.10)", color: "#141827", fontWeight: 900, fontSize: 16, textAlign: "center" as const, whiteSpace: "normal" as const, overflowWrap: "anywhere" as const };
...
const specialCostTitleStyle = { margin: "0 0 18px", color: "#a21caf", fontWeight: 900, ...ui.title };
const specialGroupCellStyle = { padding: "18px 20px", background: "#fce7f3", color: "#9d174d", fontSize: 16, fontWeight: 900, borderBottom: "1px solid rgba(190,24,93,.16)" };
const groupCellStyle = { padding: "18px 20px", background: "rgba(230,128,25,.08)", color: "#141827", fontSize: 16, fontWeight: 900, borderBottom: "1px solid rgba(255,0,135,.10)" };
const costControlsStyle = { display: "grid", gridTemplateColumns: "repeat(var(--xb-cols-4), minmax(0, 1fr))", alignItems: "end", gap: 22, marginBottom: 28, padding: 24, border: "1px solid rgba(52,64,84,.12)", borderRadius: 16, background: "rgba(255,255,255,.72)" , minWidth: 0 };
const costLabelStyle = { display: "grid", gap: 10, color: "#344054", textAlign: "center" as const , ...ui.label };
const costInputStyle = { height: 44, border: "1px solid rgba(52,64,84,.18)", background: "#fff", color: "#141827", outline: "none", boxSizing: "border-box" as const, textAlign: "center" as const , ...ui.field };
const pricingGridStyle = { display: "grid", gridTemplateColumns: "repeat(var(--xb-cols-2), minmax(0, 1fr))", gap: 28, alignItems: "stretch" , minWidth: 0 };
...
const pricingPanelStyle = { ...ui.section };
const pricingPanelHeaderStyle = { display: "grid", justifyItems: "center", gap: 12, marginBottom: 28, textAlign: "center" as const };
const subPanelTitleStyle = { margin: 0, color: "#e68019", fontWeight: 900, ...ui.title };
const greenBadgeStyle = { color: "#22c55e", fontSize: 15, fontWeight: 900 };
const pricingFieldsStyle = { display: "grid", gridTemplateColumns: "repeat(var(--xb-cols-3), minmax(0, 1fr))", gap: 24, alignItems: "center" , minWidth: 0 };
const pricingOperationalFieldsStyle = { display: "grid", gridTemplateColumns: "repeat(var(--xb-cols-4), minmax(0, 1fr))", gap: 24, alignItems: "center" , minWidth: 0 };
const paramLabelStyle = { display: "grid", justifyItems: "center", gap: 12, color: "#667085", textAlign: "center" as const , ...ui.label };
const paramInlineStyle = { display: "inline-flex", alignItems: "center", justifyContent: "center", gap: 10 };
const paramInputStyle = { width: 126, height: 64, borderRadius: 12, border: "1px solid rgba(52,64,84,.18)", background: "#fff", textAlign: "center" as const, fontSize: 25, fontWeight: 900, outline: "none", boxShadow: "none" };
const paramAffixStyle = { fontSize: 20, fontWeight: 900 };
const highlightParamLabelStyle = { ...paramLabelStyle, width: 260 , ...ui.label };
const highlightParamInlineStyle = { display: "grid", gridTemplateColumns: "42px 126px 42px", alignItems: "center", justifyContent: "center", gap: 10 , minWidth: 0 };
...
const goalCompanyTabsStyle = { display: "grid", gridTemplateColumns: "repeat(var(--xb-cols-3), minmax(0, 1fr))", ...ui.tabs, minWidth: 0 };
const goalCompanyButtonStyle = { border: "none", background: "transparent", color: "#667085", cursor: "pointer" , ...ui.button };
...
const quoteParametersGridStyle = { display: "grid", gridTemplateColumns: "repeat(var(--xb-cols-2), minmax(0, 1fr))", gap: 24, alignItems: "stretch" , minWidth: 0 };
const quoteLogoPanelStyle = { minHeight: 390, display: "grid", alignContent: "center", justifyItems: "center", gap: 18 , ...ui.frame };
const quoteFieldTitleStyle = { margin: 0, color: "#6f32d2", fontWeight: 900, textAlign: "center" as const , ...ui.title };
...
const quoteLogoFallbackStyle = { color: "#667085", fontSize: 16, fontWeight: 900, letterSpacing: 0 };
const quoteFileButtonStyle = { display: "inline-flex", alignItems: "center", justifyContent: "center", border: "none", background: "linear-gradient(135deg,#8b36e8,#e63dae,#ff3b25)", color: "#fff", cursor: "pointer" , ...ui.button };
...
const quoteFieldsPanelStyle = { display: "grid", alignContent: "start", gap: 20 , ...ui.section };
const goalsNoticeStyle = { minHeight: 48, display: "grid", placeItems: "center", padding: "0 18px", borderRadius: 12, border: "1px solid rgba(111,50,210,.16)", background: "rgba(111,50,210,.05)", color: "#6f32d2", fontSize: 15, fontWeight: 900, letterSpacing: 0, textAlign: "center" as const };
const goalsGridStyle = { display: "grid", gridTemplateColumns: "repeat(var(--xb-cols-3), minmax(0, 1fr))", gap: 20 , minWidth: 0 };
const goalCardStyle = { display: "grid", gap: 16, ...ui.frame };
const goalTitleStyle = { margin: 0, color: "#141827", fontWeight: 900, textAlign: "center" as const , ...ui.title };
const goalRangesStyle = { display: "grid", gap: 12 };
const goalRangeStyle = { minHeight: 94, padding: 14, borderRadius: 13, border: "1px solid", display: "grid", placeItems: "center", alignContent: "center", gap: 9, color: "#141827", fontSize: 14, fontWeight: 900, letterSpacing: 0, textAlign: "center" as const, boxSizing: "border-box" as const };
...
const filterInputStyle = { height: 44, border: "1px solid rgba(52,64,84,.18)", background: "#fff", color: "#141827", outline: "none", boxSizing: "border-box" as const , ...ui.field };
...
const materialFilterLabelStyle = { display: "grid", gap: 7, color: "#6f32d2", ...ui.label };
const materialFilterSelectStyle = { height: 44, border: "1px solid rgba(111,50,210,.24)", background: "#fff", color: "#141827", outline: "none" , ...ui.field };
const materialFilterResultStyle = { minHeight: 44, display: "inline-flex", alignItems: "center", padding: "0 13px", borderRadius: 7, background: "rgba(111,50,210,.08)", color: "#6f32d2", fontSize: 12, fontWeight: 900, letterSpacing: 0 };
...
const pinkCellStyle = { ...centerCellStyle, color: "#e6007e", fontSize: 16 };
const blueCellStyle = { ...centerCellStyle, color: "#0284c7", fontSize: 16 };
...
const twoColumnsStyle = { display: "grid", gridTemplateColumns: "repeat(var(--xb-cols-2), minmax(0, 1fr))", gap: 26 , minWidth: 0 };
const wideLabelStyle = { display: "grid", gap: 12, color: "#141827", ...ui.label };
const inputStyle = { width: "100%", height: 44, border: "1px solid rgba(52,64,84,.18)", background: "#fff", color: "#141827", outline: "none", boxSizing: "border-box" as const , ...ui.field };
const formActionsStyle = { display: "flex", justifyContent: "flex-end", gap: 16, marginTop: 4 , flexWrap: "wrap" as const };
const cancelButtonStyle = { border: "1px solid rgba(52,64,84,.16)", background: "#fff", color: "#141827", cursor: "pointer" , ...ui.button };
const productColumnsStyle = { display: "grid", gridTemplateColumns: "repeat(var(--xb-cols-2), minmax(0, 1fr))", gap: 24, alignItems: "start" , minWidth: 0 };
const productSideStyle = { ...ui.section };
const productSideTitleStyle = { margin: "0 0 20px", color: "#e6007e", fontWeight: 900, textAlign: "center" as const , ...ui.title };
...
const productOptionsButtonStyle = { border: "1px solid rgba(111,50,210,.28)", background: "#f5f1ff", color: "#6f32d2", cursor: "pointer" , ...ui.button };
...
const productOptionButtonStyle = { border: 0, background: "transparent", color: "#344054", textAlign: "left" as const, cursor: "pointer" , ...ui.button };
const productInfoPanelStyle = { margin: "0 0 20px", ...ui.frame };
const productInfoPanelHeaderStyle = { display: "grid", gap: 5, marginBottom: 14, color: "#e6007e", fontSize: 13, fontWeight: 900, letterSpacing: 0 };
const productPricingDetailsGridStyle = { display: "grid", gridTemplateColumns: "repeat(var(--xb-cols-2), minmax(0, 1fr))", gap: 8 , minWidth: 0 };
...
const productPanelEmptyStyle = { padding: "16px 12px", borderRadius: 9, background: "rgba(255,255,255,.72)", color: "#667085", fontSize: 11, fontWeight: 800, letterSpacing: 0 };
const productHistoryLayoutStyle = { display: "grid", gap: 14, alignItems: "start" };
const productHistoryListStyle = { display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(185px, 1fr))", gap: 9, maxHeight: 270, overflowY: "auto" as const, padding: "2px" , minWidth: 0 };
...
const productChangeEntryHeaderStyle = { display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, color: "#6f32d2", fontSize: 12, fontWeight: 900 , flexWrap: "wrap" as const };
...
const productFieldsStyle = { display: "grid", gridTemplateColumns: "repeat(var(--xb-cols-2), minmax(0, 1fr))", gap: 15 , minWidth: 0 };
const productLabelStyle = { display: "grid", gap: 7, color: "#344054", ...ui.label };
...
const productSearchSummaryStyle = { position: "relative" as const, minWidth: 170, minHeight: 46, display: "grid", alignContent: "center", justifyItems: "center", gap: 2, padding: "0 42px 0 16px", borderRadius: 10, border: "1px solid rgba(230,0,126,.18)", background: "#fff", color: "#667085", fontSize: 10, fontWeight: 900, letterSpacing: 0 };
...
const accessoryStyle = { marginBottom: 16, ...ui.frame };
const accessoryHeaderStyle = { display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, marginBottom: 14, color: "#6f32d2", fontSize: 14, letterSpacing: 0 , flexWrap: "wrap" as const };
...
const fichaListStyle = { maxWidth: "100%", overflowX: "auto" as const, border: "1px solid rgba(111,50,210,.14)", borderRadius: 14, background: "rgba(255,255,255,.72)" };
const fichaRowStyle = { display: "grid", gridTemplateColumns: "100px minmax(220px,1.8fr) minmax(150px,1fr) 145px 135px minmax(165px,1fr) auto", alignItems: "center", gap: 16, padding: "18px 24px", borderBottom: "1px solid rgba(255,0,135,.12)", background: "rgba(255,255,255,.72)", minWidth: 1440 };
const fichaColumnStyle = { display: "grid", gap: 5, minWidth: 0 };
const fichaColumnLabelStyle = { color: "#667085", fontSize: 10, fontWeight: 900, letterSpacing: .35 };
const fichaNumberStyle = { color: "#6f32d2", fontSize: 15, fontWeight: 900, overflowWrap: "anywhere" as const };
const fichaReferenceStyle = { color: "#141827", fontSize: 14, fontWeight: 900, overflowWrap: "anywhere" as const };
const fichaClientStyle = { color: "#e6007e", fontSize: 13, fontWeight: 900, overflowWrap: "anywhere" as const };
...
const fichaMetaStyle = { color: "#667085", fontSize: 12, fontWeight: 800, overflowWrap: "anywhere" as const };
const fichaActionsStyle = { display: "flex", alignItems: "center", justifyContent: "flex-end", gap: 10, flexWrap: "nowrap" as const };
const colorAddStyle = { display: "flex", alignItems: "center", gap: 10 , flexWrap: "wrap" as const };
const colorListStyle = { display: "grid", gridTemplateColumns: "repeat(var(--xb-cols-3), minmax(0, 1fr))", gap: 14 , minWidth: 0 };
const colorItemStyle = { display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, padding: "16px 18px", borderRadius: 12, border: "1px solid rgba(255,0,135,.18)", background: "rgba(255,248,252,.74)", color: "#141827", fontSize: 16, fontWeight: 900 , flexWrap: "wrap" as const };
